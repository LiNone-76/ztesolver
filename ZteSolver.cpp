...
static ZteModel *currentBranchModel = NULL;

CglPreProcess *ztePreProcessPointer = NULL;

//###########################################################################
// Forward declarations
//###########################################################################
int ZteClpUnitTest(const ZteModel &saveModel, const std::string &dirMiplib,
                   int testSwitch, const double *stuff, std::deque<std::string>,
                   int callBack(ZteModel *currentSolver, int whereFrom),
...
void ZteMain0(ZteModel &model, CbcParameters &parameters)
{
  ...
  parameters[CbcParam::ORBITAL]->setVal("lightweight");
    ...
    parameters[CbcParam::INFEASIBILITYWEIGHT]->setVal(model.getDblParam(ZteModel::CbcInfeasibilityWeight));
    parameters[CbcParam::INTEGERTOLERANCE]->setVal(model.getDblParam(ZteModel::CbcIntegerTolerance));
    parameters[CbcParam::INCREMENT]->setVal(model.getDblParam(ZteModel::CbcCutoffIncrement));
}
...
void printMessage(ZteModel &model, std::string message, int type = CBC_GENERAL)
{
    if (message.length()) {
...
void printWarning(ZteModel &model, std::string message, int type = CBC_GENERAL_WARNING)
{
    if (message.length()) {
        model.messageHandler()->message(type, model.messages())
                << message << CoinMessageEol;
...
{
    double range[] = {
            -ZTE_DBL_MAX, -1.0e15, -1.0e11, -1.0e8, -1.0e5, -1.0e4, -1.0e3,
...
static void sortOnOther(int *column, const int *rowStart, int *order,
                        int *other, int nRow, int nInRow, int where)
{
    ...
    ZteSort_2(other, other + nRow, order);
    int first = 0;
    iRow = order[0];
...
static void statistics(ClpSimplex *originalModel, ClpSimplex *model)
{
    ...
    if (model->integerInformation()) {
        ...
        for (int ifInt = 0; ifInt < 2; ifInt++) {
            for (int ifAbs = 0; ifAbs < 2; ifAbs++) {
                ...
                ZteSort_2(obj, obj + numberSort, which);
                double last = obj[0];
                ...
                if (numberDifferentObj < target) {
                    ...
                    if (saveModel) {
                        ...
                        int *columnAddDummy = new int[numberDifferentObj + 1];
                                ...
                                if (jColumn - iLast > 1) {
                                    ...
                                    for (int kColumn = iLast; kColumn < jColumn; kColumn++) {
                                        ...
                                        if (integerInformation && !integerInformation[iColumn])
                                            allInteger = false;
                                        if (lower != -ZTE_DBL_MAX) {
                                            if (lowerValue != -ZTE_DBL_MAX)
                                                lower += lowerValue;
                                            else
                                                lower = -ZTE_DBL_MAX;
                                        }
                                        if (upper != ZTE_DBL_MAX) {
                                            if (upperValue != ZTE_DBL_MAX)
                                                upper += upperValue;
                                            else
                                                upper = ZTE_DBL_MAX;
                                        }
    ...
    if (model->logLevel() == 63
        ...
        for (iColumn = 0; iColumn < numberColumns; iColumn++) {
            ...
            ZteSort_2(row + start, row + start + length, element + start);
        }
        ZteSort_2(other, other + numberColumns, order);
                ...
                if (iRow < 500000) {
                    for (int lColumn = jColumn; lColumn < kColumn; lColumn++) {
                        ...
                        if (model->logLevel() == 63) {
                            ...
                            for (int i = start; i < start + iRow; i++)
    ...
    if (model->logLevel() > 3) {
        ...
        while (iPass < 2) {
            ...
            int numberBlocks = 0;
            int thisBestBreak = -1;
            double thisBestValue = ZTE_DBL_MAX;
            ...
            for (int iRow = start; iRow != stop; iRow += increment) {
                ...
                maximumBlockSize = ZteMax(maximumBlockSize, blockCount[iBlock]);
                numberRowsDone++;
    ...
    if (model->logLevel() == 63
        ...
        for (iRow = 0; iRow < numberRows; iRow++) {
            ...
            ZteSort_2(column + start, column + start + length, element + start);
        }
        ZteSort_2(other, other + numberRows, order);
                ...
                if (iColumn < 500000) {
                    ...
                    for (int lRow = jRow; lRow < kRow; lRow++) {
                        ...
                        if (model->logLevel() == 63) {
                            ...
                            for (int i = start; i < start + iColumn; i++)
                    ...
                    ZteSort_2(randomRow, randomRow + nLook, sortRow);
                    randomRow[nLook] = ZTE_DBL_MAX;
                    double last = -ZTE_DBL_MAX;
                    int iLast = -1;
        ...
        {
            ZteSort_2(randomColumn, randomColumn + numberColumns, sortColumn);
            for (iColumn = 0; iColumn < numberColumns; iColumn++) {
                int i = sortColumn[iColumn];
                backColumn[i] = iColumn;
            }
            randomColumn[numberColumns] = ZTE_DBL_MAX;
            double last = -ZTE_DBL_MAX;
            int iLast = -1;
            ...
            ZteSort_2(randomRow, randomRow + numberRows, sortRow);
            for (iRow = 0; iRow < numberRows; iRow++) {
                int i = sortRow[iRow];
                backRow[i] = iRow;
            }
            randomRow[numberRows] = ZTE_DBL_MAX;
            last = -ZTE_DBL_MAX;
            iLast = -1;
                ...
                if (randomRow[iLook] > last) {
                    if (iLast >= 0) {
                        ...
                        if (n > 1) {
                            ...
                            ZteSort_2(order + iLast, order + iLook, sortRow + iLast);
                        }
                        for (int i = iLast; i < iLook; i++) {
                ...
                if (numberPossible > 1 && (numberIntegers || false)) {
                    ...
                    for (int jLook = iLook + 1; jLook < iLook + numberPossible; jLook++) {
                        ...
                        while (nStackR) {
                            ...
                            mapRow[row1] = row2;
                            mapRow[row2] = row1;
                            int start1 = rowStart[row1];
                            int offset2 = rowStart[row2] - start1;
                            ...
                            while (nStackC) {
                                ...
                                int start = columnStart[iColumn];
                                int offset2 = columnStart[iColumn2] - start;
                                for (int i = start; i < start + length; i++) {
                                    int iRow = row[i];
                            ...
                            for (iRow = 0; iRow < numberRows; iRow++) {
                                ...
                                if (mapRow[iRow] == iRow) {
                                    ...
                                    if (rowLower[iRow] != rowLower[row2] || rowLower[row2] != rowLower[iRow])
                                        good = false;
                                    int offset2 = rowStart[row2] - start;
                                    for (int i = start; i < start + length; i++) {
                                        int jColumn = column[i];
                                        double value = element[i];
                                        int jColumn2 = column[i + offset2];
...
static void generateCode(ZteModel * /*model*/, const char *fileName, int type,
                         int preProcess)
{
...
#define MAXONELINE 200
    ...
    strcpy(line[numberLines++], "0#include \"ZteModel.hpp\"");
    ...
    strcpy(line[numberLines++], "5  zteModel->initialSolve();");
    ...
    if (sizecode) {
        ...
        strcpy(line[numberLines++], "5  zteModel->setMinimumDrop(CoinMin(5.0e-2,");
        strcpy(
                line[numberLines++],
                "5       fabs(zteModel->getMinimizationObjValue())*1.0e-3+1.0e-4));");
        strcpy(line[numberLines++], "5  if (zteModel->getNumCols()<500)");
        strcpy(line[numberLines++],
               "5    zteModel->setMaximumCutPassesAtRoot(-100); // always do 100 "
               "if possible");
        strcpy(line[numberLines++], "5  else if (zteModel->getNumCols()<5000)");
        strcpy(
                line[numberLines++],
                "5    zteModel->setMaximumCutPassesAtRoot(100); // use minimum drop");
        strcpy(line[numberLines++], "5  else");
        strcpy(line[numberLines++],
               "5    zteModel->setMaximumCutPassesAtRoot(20);");
        strcpy(line[numberLines++], "5  zteModel->setMaximumCutPasses(1);");
    }
    if (preProcess <= 0) {
        ...
        if (preProcess) {
            ...
            strcpy(line[numberLines++], "5  zteModel->setStrategy(strategy);");
        }
    } else {
        ...
        strcpy(line[numberLines++],
               "5  OsiSolverInterface * saveSolver=zteModel->solver()->clone();");
        ...
        strcpy(line[numberLines++],
               "5  process.messageHandler()->setLogLevel(zteModel->logLevel());");
        ...
        strcpy(line[numberLines++], "5  zteModel->assignSolver(solver2);");
        strcpy(line[numberLines++], "5  zteModel->initialSolve();");
    }
    ...
    strcpy(line[numberLines++], "0  ZteModel model(solver1);");
    strcpy(line[numberLines++], "0  // Now do requested saves and modifications");
    strcpy(line[numberLines++], "0  ZteModel * zteModel = & model;");
    strcpy(line[numberLines++],
    ...
    strcpy(line[numberLines++],
           ...
           "*) NULL);");
    // add in actual solve
    strcpy(line[numberLines++], "5  zteModel->branchAndBound();");
    strcpy(
            line[numberLines++],
            "8  std::cout<<argv[1]<<\" took \"<<CoinCpuTime()-time1<<\" seconds, \"");
    strcpy(line[numberLines++],
           "8	   <<zteModel->getNodeCount()<<\" nodes with objective \"");
    strcpy(line[numberLines++], "8	   <<zteModel->getObjValue()");
    strcpy(line[numberLines++], "8	   <<(!zteModel->status() ? \" "
    ...
    strcpy(line[numberLines++],
           "5  if (zteModel->getMinimizationObjValue()<1.0e50) {");
    if (preProcess > 0) {
        strcpy(line[numberLines++], "5    // post process");
        strcpy(line[numberLines++],
               "5    process.postProcess(*zteModel->solver());");
        strcpy(line[numberLines++], "5    // Solution now back in saveSolver");
        strcpy(line[numberLines++], "5    zteModel->assignSolver(saveSolver);");
        strcpy(line[numberLines++], "5    "
                                    "memcpy(zteModel->bestSolution(),zteModel->"
    ...
    strcpy(line[numberLines++],
           "5    solver1.setColSolution(zteModel->bestSolution());");
    strcpy(line[numberLines++],
...
void checkSOS(ZteModel * /*babModel*/, const OsiSolverInterface * /*solver*/)
{
#ifdef COIN_DEVELOP
...
static void putBackOtherSolutions(ZteModel *presolvedModel, ZteModel *model,
                                  CglPreProcess *preProcess)
{
    ...
    if (numberSolutions > 1) {
        ...
        double *bestSolution = ZteCopyOfArray(presolvedModel->bestSolution(), numberColumns);
        ...
        for (int iSolution = numberSolutions - 1; iSolution >= 0; iSolution--) {
            presolvedModel->setCutoff(ZTE_DBL_MAX);
...
int ZteMain1(std::deque< std::string > inputQueue, ZteModel &model,
             CbcParameters &parameters, int callBack(ZteModel *currentSolver, int whereFrom))
{
    ClpParameters &clpParameters = parameters.clpParameters();
...
#define CBC_USE_INITIAL_TIME 1
    ...
    model_.setDblParam(ZteModel::CbcStartSeconds, CoinGetTimeOfDay());
  else
    model_.setDblParam(ZteModel::CbcStartSeconds, CoinCpuTime());
#endif
    ZteModel *babModel_ = NULL;
  ...
  glp_prob *coin_glp_prob = NULL;
    ...
    */
    ZteMessageHandler *generalMessageHandler = model_.messageHandler();
  ...
  */
  ...
  parameters.enablePrinting();
    ...
    double time0;
    double time0Elapsed = ZteGetTimeOfDay();
    {
        double time1 = ZteCpuTime(), time2;
        time0 = time1;
        ...
        const char dirsep = ZteFindDirSeparator();
...
#define MORE_CUTS
      ...
      twomirGen.setAway(0.01);
        ...
        int status, iValue;
        double dValue;
        std::string prompt = "Zte: ";

#if CBC_QUIET == 0
        if (parameters.printWelcome()) {
            buffer.str("");
            buffer << "Welcome to the ZTE MILP Solver" << std::endl;
            ...
            printMessage(model_, buffer.str());
            // Print command line
            if (!inputQueue.empty()) {
        ...
        if (!inputQueue.size()) {
            ...
            std::cout
                    << "Zte takes input from arguments ( - switches to stdin)"
                    << std::endl
        ...
        if (!numberMatches) {
          ...
          for (int iParam = ClpParam::FIRSTPARAM + 1;
            ...
            if (cbcParamCode == CbcParam::BAB && goodModel) {
                if (clqstrMode == "before") { // performing clique strengthening
                    ...
                    if (clqStr.constraintsExtended() + clqStr.constraintsDominated() > 0) {
                        ...
                        if (model_.solver()->isProvenPrimalInfeasible()) {
                            printMessage(model_, "Clique Strengthening says infeasible!");
                        } else {
                            buffer.str("");
                ...
                if (model_.useElapsedTime())
                    model_.setDblParam(ZteModel::CbcStartSeconds, ZteGetTimeOfDay());
                else
                    model_.setDblParam(ZteModel::CbcStartSeconds, ZteCpuTime());
#endif
          ...
          for (int i = 0; i < numberColumns; i++) {
            lower[i] = ZteMax(lower[i], -CBC_MAXIMUM_BOUND);
            upper[i] = ZteMin(upper[i], CBC_MAXIMUM_BOUND);
          }
#endif
                }
#endif
		...
		std::cout << std::endl;
            ...
            if (cbcParam->type() == CoinParam::paramDbl) {
                ...
                if (cbcParamCode == CbcParam::LPLOGLEVEL) {
                    ...
                    if (cbcParamCode == CbcParam::CUTPASS) {
                        ...
                        model_.messageHandler()->setLogLevel(ZteAbs(iValue));
                    } else if (cbcParamCode == CbcParam::MAXNODES) {
                        model_.setIntParam(ZteModel::CbcMaxNumNode, iValue);
                    } else if (cbcParamCode == CbcParam::MAXSOLS) {
                        model_.setIntParam(ZteModel::CbcMaxNumSol, iValue);
              ...
              iValue = 1;
                ...
                switch (clpParamCode) {
                    ...
                    case ClpParam::BARRIER: {
                        if (!goodModel) {
                            printWarning(model_, "** Current model not valid\n");
                            continue;
                        }
                        // Say not in integer
                        integerStatus = -1;
                   ...
                   relaxation unbounded 8 stopped on iterations limit
...
#if CBC_QUIET == 0
                                buffer << "Time (CPU seconds):         " << ZteCpuTime() - time0 << std::endl;
                                buffer << "Time (Wallclock Seconds):   " << ZteGetTimeOfDay() - time0Elapsed
                                       << std::endl;
#endif
                                printMessage(model_, buffer.str());
                            }
                    ...
                    case ClpParam::TIGHTEN: {
                        if (!goodModel) {
                            printMessage(model_, "** Current model not valid\n");
                            continue;
                        }
                    ...
                    case ClpParam::PLUSMINUS: {
                        ...
                        if (clpMatrix) {
                            ...
                            printMessage(model_, "Matrix not a ClpPackedMatrix");
                        }
                    } break;
                    ...
                    case ClpParam::NETWORK: {
                        ...
                        if (clpMatrix) {
                            ...
                            if (newMatrix->getIndices()) {
                                lpSolver->replaceMatrix(newMatrix);
                                delete saveMatrix;
                                printMessage(model_, "Matrix converted to network matrix");
                            } else {
                                printMessage(model_, "Matrix can not be converted to network matrix");
                            }
                        } else {
                            printMessage(model_, "Matrix not a ClpPackedMatrix");
                        }
                    ...
                    case ClpParam::FAKEBOUND: {
                        ...
                        for (iRow = 0; iRow < numberRows; iRow++) {
                            ...
                            if (rowLower[iRow] > -1.0e20 || rowUpper[iRow] < 1.0e20) {
                                rowLower[iRow] = ZteMax(rowLower[iRow], -dValue);
                                rowUpper[iRow] = ZteMin(rowUpper[iRow], dValue);
                            }
                        }
                        int iColumn;
                        int numberColumns = lpSolver->numberColumns();
                        ...
                        for (iColumn = 0; iColumn < numberColumns; iColumn++) {
                            ...
                            if (columnLower[iColumn] > -1.0e20 || columnUpper[iColumn] < 1.0e20) {
                                columnLower[iColumn] = ZteMax(columnLower[iColumn], -dValue);
                                columnUpper[iColumn] = ZteMin(columnUpper[iColumn], dValue);
                            }
                        }
                    } break;
            ...
            // int iStat2 = lpSolver->secondaryStatus();
                    ...
                    case ClpParam::GUESS:
                        if (goodModel && model_.solver()) {
                            ...
                            printWarning(model, "** Guess needs a valid model\n");
                        }
                        break;
                    default:
                        // abort();
...
#if CBC_USE_INITIAL_TIME == 1
                        if (model_.useElapsedTime())
                            model_.setDblParam(ZteModel::CbcStartSeconds,
                                               ZteGetTimeOfDay());
                        else
                            model_.setDblParam(ZteModel::CbcStartSeconds, ZteCpuTime());
#endif
               ...
               */
               ...
               comparison, scaling. Print elapsed time at the end.
                    ...
                    case CbcParam::BAB: {
                        ...
                        int truncateColumns = ZTE_INT_MAX;
                        int truncateRows = -1;
                        bool redoSOS = false;
            ...
            assert(si != NULL);
                            ...
                            if (!complicatedInteger) {
                                ...
                                if (obj) {
                                    preProcess = 0;
                                    int testOsiOptions = parameters[CbcParam::TESTOSI]->intVal();
                                    parameters[CbcParam::TESTOSI]->setVal(ZteMax(0, testOsiOptions));
                                    // create coin model
                                    ...
                                    if (linkSolver->quadraticModel()) {
                                        ...
                                        zteModel->addCutGenerator(&probing, -1, "Probing", true,
                                                                  false, false, -100, -1, -1);
                                        zteModel->cutGenerator(numCutGens++)->setTiming(true);

                                        CglGomory gomory;
                                        gomory.setLimitAtRoot(512);
                                        zteModel->addCutGenerator(&gomory, -98, "Gomory", true,
                                                                  false, false, -100, -1, -1);
                                        zteModel->cutGenerator(numCutGens++)->setTiming(true);

                                        CglKnapsackCover knapsackCover;
                                        zteModel->addCutGenerator(&knapsackCover, -98,
                                                                  "KnapsackCover", true, false,
                                                                  false, -100, -1, -1);
                                        zteModel->cutGenerator(numCutGens++)->setTiming(true);

                                        CglRedSplit redSplit;
                                        zteModel->addCutGenerator(&redSplit, -99, "RedSplit",
                                        ...
                                        zteModel->addCutGenerator(&bkClique, -98, "Clique", true,
                                                                  false, false, -100, -1, -1);
                                        zteModel->cutGenerator(numCutGens++)->setTiming(true);

                                        CglOddWheel oddWheel;
                                        oddWheel.setExtendingMethod(2);
                                        zteModel->addCutGenerator(&oddWheel, -98, "OddWheel",
                                                                  true, false, false, -100, -1,
                                                                  -1);
                                        zteModel->cutGenerator(numCutGens++)->setTiming(true);

                                        CglMixedIntegerRounding2 mixedIntegerRounding2;
                                        zteModel->addCutGenerator(&mixedIntegerRounding2, -98,
                                                                  "MixedIntegerRounding2", true,
                                                                  false, false, -100, -1, -1);
                                        zteModel->cutGenerator(numCutGens++)->setTiming(true);

                                        CglFlowCover flowCover;
                                        zteModel->addCutGenerator(&flowCover, -98, "FlowCover",
                                                                  true, false, false, -100, -1,
                                                                  -1);
                                        zteModel->cutGenerator(numCutGens++)->setTiming(true);
                                        ...
                                        zteModel->addCutGenerator(&twomir, -99, "Twomir", true,
                                                                  false, false, -100, -1, -1);
                                        zteModel->cutGenerator(numCutGens++)->setTiming(true);
                                        int heuristicOption = parameters[CbcParam::HEURISTICSTRATEGY]->modeVal();
                                        if (heuristicOption) {
                                            CbcHeuristicFPump heuristicFPump(*zteModel);
                                            ...
                                            zteModel->addHeuristic(&heuristicFPump);

                                            CbcRounding rounding(*zteModel);
                                            rounding.setHeuristicName("rounding");
                                            zteModel->addHeuristic(&rounding);

                                            CbcHeuristicLocal heuristicLocal(*zteModel);
                                            heuristicLocal.setHeuristicName("combine solutions");
                                            heuristicLocal.setSearchType(1);
                                            heuristicLocal.setFractionSmall(0.6);
                                            zteModel->addHeuristic(&heuristicLocal);

                                            CbcHeuristicGreedyCover heuristicGreedyCover(*zteModel);
                                            heuristicGreedyCover.setHeuristicName("greedy cover");
                                            zteModel->addHeuristic(&heuristicGreedyCover);

                                            CbcHeuristicGreedyEquality heuristicGreedyEquality(*zteModel);
                                            heuristicGreedyEquality.setHeuristicName("greedy equality");
                                            zteModel->addHeuristic(&heuristicGreedyEquality);
#ifdef CBC_EXPERIMENT_JJF
                                            #ifndef CBC_OTHER_SOLVER
                    CbcHeuristicRandRound heuristicRandRound(*zteModel);
                    heuristicRandRound.setHeuristicName("random rounding");
                    zteModel->addHeuristic(&heuristicRandRound);
#endif
#endif
                                        }
                                        CbcCompareDefault compare;
                                        zteModel->setNodeComparison(compare);
                                        zteModel->setNumberBeforeTrust(10);
                                        zteModel->setSpecialOptions(2);
                                        zteModel->messageHandler()->setLogLevel(1);
                                        zteModel->setMaximumCutPassesAtRoot(-100);
                                        zteModel->setMaximumCutPasses(1);
                                        zteModel->setMinimumDrop(0.05);
                  ...
                  zteModel->setNumberThreads(numberThreads % 100);
                  zteModel->setThreadMode(ZteMin((numberThreads % 1000) / 100, 7));
#endif
                                        // setCutAndHeuristicOptions(*zteModel);
                                        zteModel->branchAndBound();
                                        ...
                                        if ((linkSolver->specialOptions2() & 4) != 0 && solution) {
                                            ...
                                            storedAmpl.addCut(-ZTE_DBL_MAX, offset + 1.0e-7, n,
                                                              column, gradient);
                                            linkSolver->addRow(n, column, gradient, -ZTE_DBL_MAX,
                                                               offset + 1.0e-7);
                                            delete[] gradient;
                                            delete[] column;
                                        }
                        ...
                        if (!miplib) {
                            ...
                            if (clpSolver->status() > 0) {
                                ...
                                const char *msg[] = { "infeasible", "unbounded", "stopped",
                                                      "difficulties", "other" };
                                buffer.str("");
                                buffer << "Problem is " << msg[iStatus - 1] << " - " << ZteCpuTime() - time1a
                                       << " seconds";
                                printMessage(model_, buffer.str());
                                break;
                            }
                            ...
                            if (model_.getMaximumNodes() == -987654321) {
                                ...
                                if (!nObj) {
                                    ...
                                    const int *columnLength = matrixByCol.getVectorLengths();
                                    for (int i = 0; i < numberColumns; i++) {
                                        double value = (ZteDrand48() + 0.5) * 10000;
                                        value = 10;
                            ...
                            if (!complicatedInteger && preProcess == 0 && clpSolver->tightenPrimalBounds(0.0, 0, true) != 0) {
                                printMessage(model_, "Problem is infeasible - tightenPrimalBounds!");
                            ...
                            if (clpSolver->dualBound() == 1.0e10) {
                                ...
                                for (iRow = 0; iRow < numberRows; iRow++) {
                                    ...
                                    if (above < 1.0e12) {
                                        largest = ZteMax(largest, above);
                                    }
                                    if (below < 1.0e12) {
                                        largest = ZteMax(largest, below);
                                    }
                                    if (rowScale) {
                                    ...
                                    if (above < 1.0e12) {
                                        largestScaled = ZteMax(largestScaled, above);
                                    }
                                    if (below < 1.0e12) {
                                        largestScaled = ZteMax(largestScaled, below);
                                    }
                                }
              ...
              printMessage(model_, buffer.str());
                                ...
                                clpSolver->setDualBound(
                                        ZteMax(1.0001e8, ZteMin(100.0 * largest, 1.00001e10)));
                            }
                            si->resolve(); // clean up
#endif
                        }
                        ...
                        if (!defaultSettings) {
                            ...
                            if (tightenFactor && !complicatedInteger) {
                                if (modelC->tightenPrimalBounds(tightenFactor) != 0) {
                                    printMessage(model_, "Problem is infeasible!");
                                    model_.setProblemStatus(0);
                                    model_.setSecondaryStatus(1);
                                    // and in babModel if exists
                                    if (babModel_) {
          ...
          babModel_->assignSolver(solver3);
                        ...
                        if (mipStartBefore.size()) {
                            ...
                            if (!status) {
                                double newCutoff = ZteMin(babModel_->getCutoff(), obj + 1.0e-4);
                        ...
                        if (preProcess) {
                            ...
                            if (iGenerator < numberGenerators) {
                                preProcess = 0;
                                printMessage(model_, "PreProcessing switched off due to lazy constraints");
                            }
                        }
                        if (preProcess && cbcParamCode == CbcParam::BAB) {
                            ...
                            OsiSolverInterface *solver2;
                            {
                                ...
                                generator1.setMaxProbeRoot(
                                        ZteMin(3000, saveSolver->getNumCols()));
                  ...
                  generator1.setMaxElements(1000);
                                    ...
                                    if ((tune2 & 512) != 0)
                                        generator1.setMaxLookRoot(ZteMin(saveSolver->getNumCols(), 1000));
                                    else
                                        generator1.setMaxLookRoot(ZteMin(saveSolver->getNumCols(), 400));
                                }
                                ...
                                if (numberSOS && doSOS) {
                                    ...
                                    for (int iSet = 0; iSet < numberSOS; iSet++) {
                                        if (sosType[iSet] != 1) {
                                            ...
                                            double nonzeroValue = ZTE_DBL_MAX;
                                    ...
                                    // Create model which uses 0-1 variables
		    {
		      ...
		      int * starts = new int [2*numberLotSizing+1];
		      // add 0-1 variables
			...
			els[2*n+1] = -ZteMin(10000.0,columnUpper[iColumn]);
			cols[2*n] = iColumn;
                  ...
                  process.setApplicationData(
                    const_cast< double * >(debugValues));
                                    ...
                                    if (parameters[CbcParam::DEBUGFILE]->fileName() == "unitTest") {
                                        ...
                                        debugValues = ZteCopyOfArray(debugger->optimalSolution(),
                                                                     numberDebugValues);
                                    }
                                    redoSOS = true;
              ...
              solver2 = process.preProcessNonDefault(
                *saveSolver, translate[preProcess], numberPasses,
                tunePreProcess);
                            ...
                            if (!solver2) {
                                printMessage(model_, "Pre-processing says infeasible or unbounded");
                                // say infeasible for solution
                                integerStatus = 6;
                                delete saveSolver;
                                saveSolver = NULL;
                            ...
                            if (model_.bestSolution()) {
                                ...
                                int numberColumns = ZteMin(solver2->getNumCols(), babModel_->getNumCols());
                            ...
                            {
              OsiClpSolverInterface *solver = dynamic_cast< OsiClpSolverInterface * >(solver2);
                                    ...
                                    for (int i = 0; i < n; i++) {
                                        ...
                                        if (iColumn >= 0) {
                                            if (columnLower[iColumn] < lotsize[i].low && columnUpper[iColumn] > lotsize[i].low - 1.0e-8) {
                                                ...
                                                objects[numberLotSizing++] = new CbcLotsize(babModel_, iColumn, 2,
                                                                                            points, true);
                                            } else {
                            ...
                            if (threshold || (more2 & (512 | 1024)) != 0) {
                                ...
                                if ((more2 & (512 | 1024)) != 0) {
                                            ...
                                            if (objective[iColumn]) {
                                                if (!solver2->isInteger(iColumn)) {
                                                    ...
                                                    if (fabs(value - nearest) > 1.0e-8) {
                                                        ...
                                                        newColumn[n++] = iColumn;
                                                        if (nearest > 0.0) {
                                                            newLower += ZteMax(columnLower[iColumn], -1.0e20) * nearest;
                                                            newUpper += ZteMin(columnUpper[iColumn], 1.0e20) * nearest;
                                                        } else {
                                                            newUpper += ZteMax(columnLower[iColumn], -1.0e20) * nearest;
                                                            newLower += ZteMin(columnUpper[iColumn], 1.0e20) * nearest;
                                                        }
                                                    }
                                                }
                                            }
                                    ...
                                    if (addSlacks) {
                                        ...
                                        const int *rowStart = matrix->getVectorStarts();
                                        ...
                                        int *newColumnStart = new int[numberRows + 1];
                                        newColumnStart[0] = 0;
                                        ...
                                        double *oldObjective = ZteCopyOfArray(
                                        ...
                                        for (iRow = 0; iRow < numberRows; iRow++) {
                                            if (rowLower[iRow] != rowUpper[iRow]) {
                                                ...
                                                if (allInteger) {
                                                    ...
                                                    if (moveObj && constantObjective != ZTE_DBL_MAX) {
                                                    ...
                                                    newColumnLower[addSlacks] = ZteMax(newLower, ceil(rowLower[iRow]));
                                                    ;
                                                    newColumnUpper[addSlacks] = ZteMin(newUpper, floor(rowUpper[iRow]));
                                                    addSlacks++;
                                                }
                                            }
                                        }
                                        if (addSlacks) {
                                            ...
                                            truncatedRhsLower = ZteCopyOfArray(solver2->getRowLower(), numberRows);
                                            truncatedRhsUpper = ZteCopyOfArray(solver2->getRowUpper(), numberRows);
            ...
            if (babModel_->useElapsedTime())
              babModel_->setDblParam(ZteModel::CbcStartSeconds,
                ZteGetTimeOfDay());
            else
              babModel_->setDblParam(ZteModel::CbcStartSeconds,
                ZteCpuTime());
              // babModel_->setMaximumSeconds(timeLeft - (ZteCpuTime() -
              // time2));
#endif
                        }
                        ...
                        if (debugValues) {
                            ...
                            babModel_->solver()->activateRowCutDebugger(
                                    problemName.c_str());
                            twomirGen.probname_ = ZteStrdup(problemName.c_str());
                            // checking seems odd
                        ...
                        int bothFlags = ZteMax(ZteMin(experimentFlag, 1), strategyFlag);
...
#define ALL_LAGRANGEAN 1
                        ...
                        if (probingMode) {
                            ...
                            probingGen.setMaxProbeRoot(ZteMin(2000, numberColumns));
            ...
            switches[numberGenerators++] = 0;
                        ...
                        if (!miplib) {
                            ...
                            babModel_->setMinimumDrop(ZteMin(5.0e-2, minimumDrop));
                            if (cutPass == -1234567) {
          ...
          ;
              ...
              babModel_->solver()->activateRowCutDebugger(debugSolution);
                        ...
                        babModel_->setCutoffIncrement(
                                ZteMax(babModel_->getCutoffIncrement(), increment));
                       ...
                       about direction */
                            ...
                            if (solutionIn && useSolution >= 0) {
                                ...
                                if (preProcess) {
                                    ...
                                    int nSmaller = ZteMin(n, numberOriginalColumns);
                            ...
                            if (!testOsiSolver) {
                                ...
                                if (preProcess && (process.numberSOS() || babModel_->numberObjects())) {
                                    ...
                                    int n = ZteMin(truncateColumns, numberColumns);
                                    // allow for empty problem
                                    n = (n) ? originalColumns[n - 1] + 1 : 0;
                                    n = ZteMax(n,
                                               ZteMax(numberColumns, numberOriginalColumns));
                                    int *newColumn = new int[n];
                                    int i;
                                    for (i = 0; i < numberOriginalColumns; i++)
                                        newColumn[i] = -1;
                                    for (i = 0; i < ZteMin(truncateColumns, numberColumns);
                                         i++)
                                        newColumn[originalColumns[i]] = i;
                                    int nMissing = 0;
                                    for (int iObj = 0; iObj < numberOldObjects; iObj++) {
                  ...
                  obj->setPreferredWay(way);
                           ...
                           x-x!
                                ...
                                if (useCosts && testOsiOptions >= 0) {
                                    ...
                                    ZteSort_2(dsort, dsort + numberColumns, sort);
                                    int level = 0;
                                    double last = -1.0e100;
                                    for (int i = 0; i < numberColumns; i++) {
                                        int iPut = sort[i];
            ...
            if (numberThreads > 100) {
              ...
              if (numberThreads < 1000) {
                ...
                int multiplier = ZteMax((numberThreads % 1000) / 100, 1);
                babModel_->setThreadMode(1);
                                ...
                                */
            {
              ...
              if (clpSolver && clpSolver->numberSOS()) {
                ...
                ClpSimplex *fakeSimplex = new ClpSimplex(*clpSolver->getModelPtr());
				    ...
				    int * starts =
				      new int[ZteMax(numberSOS+1,numberRows)];
                ...
                for (int i = 0; i < numberSOS; i++) {
                  ...
                  for (int j = 0; j < n; j++) {
                    ...
                    rhs = ZteMax(upper[iColumn] + previous, rhs);
                    if (type == 2)
                            ...
                            if (multipleRoot < 10000) {
                                ...
                                for (int iGo = 0; iGo < numberGoes; iGo++) {
                                    ...
                                    if (tempModel.bestSolution()) {
                                        bestSolutions[numberSolutions] = ZteCopyOfArray(
                                ...
                                ;
                                ZteSort_2(bestValues, bestValues + numberSolutions, which);
...
#define MAX_NAUTY_PASS 2000
...
#endif
            void printHistory(const char *file /*,ZteModel * model*/);
            ...
            babModel_->setThreadMode((numberThreads % 1000) / 100);
                   ...
                   directory location could be passed to CbcClpUnitTest. */
                     ...
                     */
                            ...
                            if (dirMiplib != "") {
                                int returnCode = ZteClpUnitTest(model_, dirMiplib, extra2, stuff,
          ...
          for (iGenerator = 0; iGenerator < babModel_->numberHeuristics();
            ...
            if (heuristic->numRuns()) {
              ...
              printMessage(model_, buffer.str());
                        ...
                        time2 = ZteCpuTime() + ZteCpuTimeJustChildren();
                        totalTime += time2 - time1;
                        // For best solution
                ...
                babModel_->solver());
                                ...
                                for (int i = 0; i < n; i++) {
                                    lower2[i] = ZTE_DBL_MAX;
                                    upper2[i] = -ZTE_DBL_MAX;
                                }
                  ...
                  if (saveSolver) {
                    ...
                    originalSolver->writeMps("infB");
...
#endif
                            checkSOS(babModel_, babModel_->solver());
                        } else if (model_.bestSolution() && cbcParamCode == CbcParam::BAB && model_.getMinimizationObjValue() < 1.0e50 && preProcess) {
                            buffer.str("");
                        ...
                        if (cbcParamCode == CbcParam::BAB) {
                            ...
                            if (parameters[CbcParam::DEBUGFILE]->fileName() == "create" && bestSolution) {
              ...
              model_.solver()->setColSolution(bestSolution);
                            ...
                            statistics_sys_seconds = ZteSysTime();
                            statistics_elapsed_seconds = ZteWallclockTime();
		       ...
		       so back to sprintf */
                            ...
                            buffer << "Time (CPU seconds):             "
                                   << ZteCpuTime() - time1 << std::endl;
                            buffer << "Time (Wallclock seconds):       "
                                   << ZteGetTimeOfDay() - time1Elapsed << std::endl;
#endif
                            printMessage(model_, buffer.str());
                            int returnCode = 0;
                            if (callBack != NULL)
            ...
            goodModel = true;
                        ...
                        if (preSolve) {
                            ...
                            if (!linkSolver || !linkSolver->quadraticModel()) {
                                ...
                                if (model2->lengthNames()) {
                                    ...
                                    for (iRow = 0; iRow < numberRows; iRow++) {
                                        rowNames[iRow] = ZteStrdup(model2->rowName(iRow).c_str());
                                    }

                                    columnNames = new char *[numberColumns];
                                    for (iColumn = 0; iColumn < numberColumns; iColumn++) {
                                        columnNames[iColumn] = ZteStrdup(model2->columnName(iColumn).c_str());
                                    }
                                }
                                // see if extension lp
                                bool writeLp = false;
                            ...
                            if (good) {
                                ...
                                if (got[6] >= 0) {
                                    solutionIn = reinterpret_cast< double * >(
                                            malloc(numberColumns * sizeof(double)));
                                    for (iColumn = 0; iColumn < numberColumns; iColumn++)
                                        solutionIn[iColumn] = -ZTE_DBL_MAX;
                                }
                                ...
                                for (iColumn = 0; iColumn < numberColumns; iColumn++) {
                                    columnNames[iColumn] = ZteStrdup(lpSolver->columnName(iColumn).c_str());
                                    // if (got[5]!=-1)
                                ...
                                int nLine = 0;
                                iColumn = -1;
                                int lowestPriority = -ZTE_INT_MAX;
                                bool needCard = true;
                                    ...
                                    *put = '\0';
                                    ...
                                    for (int i = 0; i < nAcross; i++) {
                                        ...
                                        switch (order[i]) {
                                            ...
                                            case 3:
                                                pri = atoi(pos);
                                                lowestPriority = ZteMax(lowestPriority, pri);
                                                break;
                                                // up
                                            case 4:
                                                up = atof(pos);
                    ...
                    case CbcParam::DEBUG: {
                        ...
                        if (fileName == "create" || fileName == "createAfterPre") {
                            ...
                            printMessage(model_, "Will create a debug file so "
                                                 "this run should be a good one");
                            break;
                        } else if (fileName == "unitTest") {
                            printMessage(model_, "debug will be done using file "
                                                 "name of model");
                            parameters[CbcParam::DEBUGFILE]->setFileName(fileName);
                            break;
                        }
          ...
          debugNumberColumns = numberDebugValues;
                    ...
                    case CbcParam::WRITEMODEL: {
                        ...
                        if (!status) {
                            ...
                            // errors
                            printMessage(model_, "There were errors on output");
                        }
                    } break;
                    ...
                    case CbcParam::UNITTEST: {
                        ...
                        if (dirMiplib != "") {
                            returnCode = ZteClpUnitTest(model_, dirMiplib, -3, NULL,
                                                        saveInputQueue, callBack, parameters);
                        } else if (dirSample != "") {
                            returnCode = ZteClpUnitTest(model_, dirSample, -2, NULL,
                                                        saveInputQueue, callBack, parameters);
                        } else {
                            buffer.str("");
          ...
          if (info && statusUserFunction_[0]) {
            ...
            if (objectiveValue < 1.0e50) {
              ...
              ZteCopyN(lpSolver->primalColumnSolution(), n,
                info->primalSolution);
              int numberRows = lpSolver->numberRows();
              free(info->dualSolution);
              info->dualSolution = (double *)malloc(numberRows * sizeof(double));
              ZteCopyN(lpSolver->dualRowSolution(), numberRows,
                info->dualSolution);
            } else {
...
clp watson.mps -\nscaling off\nprimalsimplex");
	    std::endl;
                    ...
                    case CbcParam::WRITESTATS: {
                        ...
                        fprintf(fp, ",");
                        for (int i = 0; i < inputQueue.size(); i++) {
                            if (strstr(inputQueue[i].c_str(), ".gz") || strstr(inputQueue[i].c_str(), ".mps")) {
                                continue;
                            }
              ...
              glp_free_env();
                        ...
                        size_t lengthPrint = static_cast< size_t >(ZteMax(lengthName, 8));
                        ...
                        if (doMask) {
                            ...
                            ZteSort_2(sort, sort + nEntries, masks);
                            int lastLength = -1;
                            for (i = 0; i < nEntries; i++) {
                                int length = sort[i];
                            ...
                            for (iColumn = 0; iColumn < numberColumns; iColumn++) {
                                if (printMode == 13 || model_.solver()->isInteger(iColumn)) {
                                    ...
                                    CoinConvertDouble(5, 2,
                                                      ZteMax(-1.0e30, columnLower[iColumn]),
                                                      outputValue);
                                    fprintf(fp, "  %s\n", outputValue);
                                    ...
                                    for (; i < lengthPrint; i++)
                                        fprintf(fp, " ");
                                    CoinConvertDouble(5, 2,
                                                      ZteMin(1.0e30, columnUpper[iColumn]),
                                                      outputValue);
                                    fprintf(fp, "  %s\n", outputValue);
                                }
                            }
...
#if CBC_QUIET == 0
    buffer.str("");
    buffer << "Total time (CPU seconds):       " << ZteCpuTime() - time0
           << "   (Wallclock seconds):       "
           << ZteGetTimeOfDay() - time0Elapsed << std::endl;
    printMessage(model_, buffer.str());
#endif
#ifdef COINUTILS_HAS_GLPK
    if (coin_glp_prob) {
    ...
    buffer.str("");
    buffer << "Total time " << ZteCpuTime() - time0;
