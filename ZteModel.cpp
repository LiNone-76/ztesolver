...
#include "ZteModel.h"
#include "ZteCglDuplicateRow.h"
#include "ZteGeneralDepth.h"
#include "tools.h"
...
ZteModel::ZteModel()
        : solver_(NULL), ownership_(0x80000000), numberStrongIterations_(0),
          specialOptions_(0), moreSpecialOptions_(0),moreSpecialOptions2_(0),
          numberObjects_(0), object_(NULL), ownObjects_(true), status_(-1),
          secondaryStatus_(-1), originalContinuousObjective_(ZTE_DBL_MAX),
          integerVariable_(NULL), integerInfo_(NULL), eventHappened_(false),
          hotstartSolution_(NULL), hotstartPriorities_(NULL), phase_(0),
          numberBeforeTrust_(10), continuousObjective_(ZTE_DBL_MAX),
          numberAnalyzeIterations_(0), numberSolves_(0), numberCutGenerators_(0),
          generator_(NULL), numberSavedSolutions_(0), numberStrong_(5),
          cutoffRowNumber_(-1), continuousSolver_(NULL), bestObjective_(ZTE_DBL_MAX),
          numberHeuristics_(0), fastNodeDepth_(-1), numberThreads_(0),
          bestPossibleObjective_(ZTE_DBL_MAX), sumChangeObjective1_(0.0),
          sumChangeObjective2_(0.0), numberSolutions_(0), numberHeuristicSolutions_(0),
          stateOfSearch_(0), currentSolution_(NULL), testSolution_(NULL),
          numberRowsAtContinuous_(0), maximumWhich_(INITIAL_MAXIMUM_WHICH), maximumRows_(0),
          whichGenerator_(NULL), maximumNumberCuts_(0), currentNumberCuts_(0),
          currentDepth_(0), maximumDepth_(0), lastDepth_(0), lastNumberCuts_(NULL),
          lastNumberCuts2_(0), maximumCuts_(0), numberOldActiveCuts_(0), numberNewCuts_(0),
          numberNodes_(0), numberNodes2_(0), numberIterations_(0), maximumStatistics_(0),
          maximumDepthActual_(0), numberDJFixed_(0.0), multipleRootTries_(0),
          randomSeed_(-1), stoppedOnGap_(false), statistics_(NULL),
          maximumCutPassesAtRoot_(20), originalColumns_(NULL), numberFixedAtRoot_(0),
          numberFixedNow_(0), analyzeResults_(NULL), continuousInfeasibilities_(ZTE_INT_MAX),
          continuousSolution_(NULL), usedInSolution_(NULL), numberLongStrong_(0),
          lastSecPrintProgress_(0.0), printFrequency_(1), secsPrintFrequency_(1),
          numberExtraIterations_(0), maximumNumberIterations_(-1), numberStoppedSubTrees_(0),
          numberExtraNodes_(0), atSolutionSolver_(NULL), globalConflictCuts_(NULL),
          heuristic_(NULL), addedCuts_(NULL), walkback_(NULL), lastNodeInfo_(NULL),
          lastCut_(NULL), storedRowCuts_(NULL), lastHeuristic_(NULL), topOfTree_(NULL),
          nextRowCut_(NULL), referenceSolver_(NULL), threadMode_(0), virginGenerator_(NULL),
          defaultHandler_(true), continuousPriority_(ZTE_INT_MAX), roundIntVars_(false),
          lastNodeImprovingFeasSol_(-1), lastTimeImprovingFeasSol_(0), maximumSavedSolutions_(0),
          savedSolutions_(NULL), problemType_(0), currentPassNumber_(0),
          numberInfeasibleNodes_(0), minimumDrop_(1.0e-7), howOftenGlobalScan_(3),
          numberGlobalViolations_(0), resolveAfterTakeOffCuts_(true), whenCuts_(-1),
          maximumCutPasses_(10), numberUpdateItems_(0), maximumNumberUpdateItems_(0),
          updateItems_(NULL), numberFathoms_(0), master_(NULL), masterThread_(NULL),
          searchStrategy_(-1), strongStrategy_(0), emptyWarmStart_(NULL), heuristicModel_(NULL),
          preProcess_(NULL){
    //preModel = CbcModel();
    parentModel_ = NULL;
    ...
    intParam_[CbcMaxNumNode] = ZTE_INT_MAX;
    intParam_[CbcMaxNodesNotImproving] = ZTE_INT_MAX;
    intParam_[CbcMaxNumSol] = ZTE_INT_MAX / 2;
    ...
    problemFeasibility_ = new ZteFeasibilityBase();
    ...
    handler_ = new ZteMessageHandler();
    handler_->setLogLevel(2);
    messages_ = CbcMessage();
    eventHandler_ = new ZteEventHandler();
    appData_ = NULL;
}
//ZteModel::ZteModel(const ZteOsiSolverInterface &rhs)
ZteModel::ZteModel(const OsiSolverInterface &rhs)
        : numberStrongIterations_(0),specialOptions_(0),
          moreSpecialOptions_(0), moreSpecialOptions2_(0),numberObjects_(0),
          object_(NULL), ownObjects_(true), status_(-1), secondaryStatus_(-1),
          originalContinuousObjective_(ZTE_DBL_MAX), eventHappened_(false),
          hotstartSolution_(NULL), hotstartPriorities_(NULL), phase_(0),
          numberBeforeTrust_(10), integerInfo_(NULL), preferredWay_(0),
          continuousObjective_(ZTE_DBL_MAX), numberAnalyzeIterations_(0),
          numberSolves_(0), numberCutGenerators_(0), generator_(NULL),
          numberSavedSolutions_(0), numberStrong_(5), cutoffRowNumber_(-1),
          continuousSolver_(NULL), bestObjective_(ZTE_DBL_MAX), numberHeuristics_(0),
          fastNodeDepth_(-1), numberThreads_(0), bestPossibleObjective_(ZTE_DBL_MAX),
          sumChangeObjective1_(0.0), sumChangeObjective2_(0.0), numberSolutions_(0),
          numberHeuristicSolutions_(0), stateOfSearch_(0), numberRowsAtContinuous_(0),
          maximumWhich_(INITIAL_MAXIMUM_WHICH), maximumRows_(0), whichGenerator_(NULL),
          maximumNumberCuts_(0), currentNumberCuts_(0), currentDepth_(0),
          maximumDepth_(0), lastDepth_(0), lastNumberCuts_(NULL), lastNumberCuts2_(0),
          maximumCuts_(0), numberOldActiveCuts_(0), numberNewCuts_(0), numberNodes_(0),
          numberNodes2_(0), numberIterations_(0), maximumStatistics_(0),
          maximumDepthActual_(0), numberDJFixed_(0.0), multipleRootTries_(0),
          randomSeed_(-1), stoppedOnGap_(false), statistics_(NULL),
          maximumCutPassesAtRoot_(20), originalColumns_(NULL), numberFixedAtRoot_(0),
          numberFixedNow_(0), analyzeResults_(NULL), continuousInfeasibilities_(ZTE_INT_MAX),
          numberLongStrong_(0), lastSecPrintProgress_(0.0), printFrequency_(1),
          secsPrintFrequency_(1), numberExtraIterations_(0), maximumNumberIterations_(-1),
          numberStoppedSubTrees_(0), numberExtraNodes_(0), atSolutionSolver_(NULL),
          globalConflictCuts_(NULL), heuristic_(NULL), addedCuts_(NULL), walkback_(NULL),
          lastNodeInfo_(NULL), lastCut_(NULL), storedRowCuts_(NULL), lastHeuristic_(NULL),
          topOfTree_(NULL), nextRowCut_(NULL), threadMode_(0), virginGenerator_(NULL),
          defaultHandler_(true), continuousPriority_(ZTE_INT_MAX), roundIntVars_(false),
          lastNodeImprovingFeasSol_(-1), lastTimeImprovingFeasSol_(0), maximumSavedSolutions_(0),
          savedSolutions_(NULL), problemType_(0), currentPassNumber_(0),
          numberInfeasibleNodes_(0), minimumDrop_(1.0e-7), howOftenGlobalScan_(3),
    ...
    solver_ = dynamic_cast<OsiSolverInterface *>(rhs.clone());
    ...
    if (numberColumns) {
        ...
        continuousSolution_ = ZteCopyOfArray(solver_->getColSolution(), numberColumns);
        usedInSolution_ = new int[numberColumns];
        ZteZeroN(usedInSolution_, numberColumns);
    ...
    branchingMethod_ = NULL;
    handler_ = new ZteMessageHandler();
    if (!solver_->defaultHandler() && solver_->messageHandler() &&
        solver_->messageHandler()->logLevel(0) != -1000)
        passInMessageHandler(dynamic_cast<ZteMessageHandler *>(solver_->messageHandler()));
    handler_->setLogLevel(2);
...
ZteModel::ZteModel(const ZteModel &rhs, bool cloneHandler)
        :
        ...
        status_(rhs.status_), secondaryStatus_(rhs.secondaryStatus_),
        ...
        maximumWhich_(rhs.maximumWhich_), maximumRows_(0),
        whichGenerator_(NULL), currentDepth_(0),
        ...
        maximumStatistics_(0), maximumDepthActual_(0),
        numberDJFixed_(0.0),randomSeed_(rhs.randomSeed_),
        ...
        atSolutionSolver_(NULL), globalConflictCuts_(NULL),
        storedRowCuts_(NULL), topOfTree_(NULL),
        ...
        updateItems_(NULL), numberFathoms_(0),
        master_(NULL), masterThread_(NULL),
        searchStrategy_(rhs.searchStrategy_),
        strongStrategy_(rhs.strongStrategy_),
        emptyWarmStart_(NULL), heuristicModel_(NULL),
        preProcess_(rhs.preProcess_){
    //preModel = rhs.preModel;
    ...
    if (numberIntegers_) {
        ...
        integerInfo_ = ZteCopyOfArray(rhs.integerInfo_, solver_->getNumCols());
    } else {
        integerVariable_ = NULL;
        integerInfo_ = NULL;
    }
    ...
    if (rhs.hotstartSolution_) {
        int numberColumns = solver_->getNumCols();
        hotstartSolution_ = ZteCopyOfArray(rhs.hotstartSolution_, numberColumns);
        hotstartPriorities_ = ZteCopyOfArray(rhs.hotstartPriorities_, numberColumns);
    } else {
    ...
    if (maximumSavedSolutions_ && rhs.savedSolutions_) {
        ...
        for (int i = 0; i < maximumSavedSolutions_; i++)
            savedSolutions_[i] =
                    ZteCopyOfArray(rhs.savedSolutions_[i], numberColumns + 2);
    } else {
    ...
    if (cloneHandler && !defaultHandler_) {
        ...
        /* We have to clone handlers - otherwise will all be
               ...
               have to sychronize */
        ZteMessageHandler *handler = rhs.handler_->clone();
    ...
    if (probingInfo_) {
        ...
        for (int i = 0; i < numberCutGenerators_; i++) {
            ...
            if (!cutGen) {
                ...
                delete generator_[i];
                cutGen =
                        dynamic_cast<CglImplication *>(virginGenerator_[i]->generator());
                assert(cutGen);
                cutGen->setProbingInfo(NULL);
                delete virginGenerator_[i];
            }
...
static void flipSolver(OsiSolverInterface *solver, double newCutoff) {
    if (solver) {
        ...
        double *array = ZteCopyOfArray(solver->getObjCoefficients(), numberColumns);
...
int ZteModel::numberSavedSolutions() const {
    if (!bestSolution_)
        return 0;
    else
...
const double *ZteModel::savedSolution(int which) const {
    if (which == 0) {
...
double ZteModel::savedSolutionObjective(int which) const {
    if (which == 0) {
...
void ZteModel::deleteSavedSolution(int which) {
...
void ZteModel::setBestObjectiveValue(double objectiveValue) {
    bestObjective_ = objectiveValue;
}

// Delete best and saved solutions
void ZteModel::deleteSolutions() {
    delete[] bestSolution_;
    bestSolution_ = NULL;
    for (int i = 0; i < maximumSavedSolutions_; i++) {
        delete[] savedSolutions_[i];
...
void ZteModel::passInMessageHandler(ZteMessageHandler *handler) {
    if (defaultHandler_) {
        delete handler_;
        handler_ = NULL;
    }
...
*/
void ZteModel::synchronizeModel() {
    if (!numberObjects_)
        return;
    int i;
    for (i = 0; i < numberHeuristics_; i++)
    ...
    if (!solverCharacteristics_) {
        ...
        if (solverCharacteristics) {
            ...
            // 补正
            this->solver()->setAuxiliaryInfo(&defaultC);
...
void ZteModel::setPointers(const OsiSolverInterface *solver) {
    //preModel.setPointers(solver);
    ...
    dblParam_[CbcCurrentMinimizationObjectiveValue] =
            dblParam_[CbcCurrentObjectiveValue] * dblParam_[CbcOptimizationDirection];
}

// Add one generator
void ZteModel::addCutGenerator(CglCutGenerator *generator, int howOften,
...
*/
void ZteModel::convertToDynamic() {
    int iObject;
    ...
    for (iObject = 0; iObject < numberObjects_; iObject++) {
        ...
        if (obj1 && !obj2) {
            ...
            double costValue = ZteMax(1.0e-5, fabs(cost[iColumn]));
...
#define BRANCH_BREAKEVEN 0.3
    ...
    if (branchingMethod_) {
        if ((branchingMethod_->whichMethod() & 1) == 0 &&
            !branchingMethod_->chooseMethod()) {
...
void ZteModel::synchronizeNumberBeforeTrust(int type) {
    int iObject;
    for (iObject = 0; iObject < numberObjects_; iObject++) {
        CbcSimpleIntegerDynamicPseudoCost *obj2 =
                dynamic_cast<CbcSimpleIntegerDynamicPseudoCost *>(object_[iObject]);
        if (obj2) {
            ...
            if (!type) {
                ...
                value = (value * 11) / 10 + 1;
                value = ZteMax(numberBeforeTrust_, value);
                obj2->setNumberBeforeTrust(value);
            } else {
                assert(type == 2);
                int value = obj2->numberBeforeTrust();
                int n = ZteMax(obj2->numberTimesDown(), obj2->numberTimesUp());
                if (n >= value) {
                    value = ZteMin(ZteMin(n + 1, 3 * (value + 1) / 2),
...
void ZteModel::findIntegers(bool startAgain, int type) {
    assert(solver_);
    /*
    ...
    ZteZeroN(mark, numberColumns);
    int iPriority = -100000;
    for (iObject = 0; iObject < numberObjects_; iObject++) {
        iPriority = ZteMax(iPriority, object_[iObject]->priority());
...
void ZteModel::reserveCurrentSolution(const double *solution) {
    int numberColumns = getNumCols();
    if (!currentSolution_)
        currentSolution_ = new double[numberColumns];
    testSolution_ = currentSolution_;
...
void ZteModel::setCutoff(double value) {
#ifdef JJF_ZERO
    double tol = 0;
...
OsiBranchingInformation ZteModel::usefulInformation() const {
    ...
    usefulInfo.numberBranchingSolutions_ =
...
void ZteModel::makeGlobalCut(const OsiColCut *cut) {
    const double *lower;
    const double *upper;
    if (topOfTree_) {
...
void ZteModel::makeGlobalCut(const OsiColCut &cut)
{
    ...
    for (int i = 0; i < nLower; i++) {
        int iColumn = indexLower[i];
        double newValue = ZteMax(lower[iColumn], boundLower[iColumn]);
        if (topOfTree_)
            topOfTree_->setColLower(iColumn, newValue);
        else
            solver_->setColLower(iColumn, newValue);
    ...
    for (int i = 0; i < nUpper; i++) {
        int iColumn = indexUpper[i];
        double newValue = ZteMin(upper[iColumn], boundUpper[iColumn]);
        if (topOfTree_)
            topOfTree_->setColUpper(iColumn, newValue);
        else
            solver_->setColUpper(iColumn, newValue);
...
int ZteModel::makeGlobalCut(const OsiRowCut *cut) {
    if (cut->row().getNumElements() > 1 - 1) {
        ...
        if (topOfTree_) {
            lower = topOfTree_->lower();
            upper = topOfTree_->upper();
            lb = ZteMax(lb, lower[iColumn]);
            topOfTree_->setColLower(iColumn, lb);
            ub = ZteMin(ub, upper[iColumn]);
...
void ZteModel::makeGlobalCuts(int number, const int *which)
{
    ...
    const int *rowStart = solver_->getMatrixByRow()->getVectorStarts();
...
*/
double ZteModel::checkSolution(double cutoff, double *solution,
...
#define CBC_LEAVE_CRUNCH_ON_CHECK_SOLUTION // for now
      ...
      clpContinuousSolver->setSpecialOptions(saveSpecialOptions &
        ...
        assert(basis != NULL);
        double *saveSolution =
                ZteCopyOfArray(solver_->getColSolution(), solver_->getNumCols());
      ...
      const CoinBigIndex *columnStart =
          solver_->getMatrixByCol()->getVectorStarts();
      ...
      for (int i = 0; i < numberRows; i++) {
        ...
        double infeasibility =
            CoinMax(rowActivity[i] - rowUpper[i], rowLower[i] - rowActivity[i]);
      ...
      if (largestInfeasibility > 10.0 * primalTolerance)
        ...
        if ((specialOptions_ & 4) == 0 && (moreSpecialOptions2_ & 10) != 8) {
            ...
            bool allSlack =
                    (specialOptions_ & 2) == 0 && solverCharacteristics_->warmStart();
            if (numberIntegers_ * 4 > solver_->getNumCols() ||
                solver_->getNumCols() < 10000)
                allSlack = true;
            if (allSlack) {
                /*
        ...
        if (objectiveValue > cutoff &&
            objectiveValue < cutoff + 1.0e-8 + 1.0e-8 * fabs(cutoff))
            cutoff = objectiveValue; // relax
        if ((solver_->isProvenOptimal() || (specialOptions_ & 4) != 0) &&
            objectiveValue <= cutoff) {
      ...
      int nNotNeeded = 0;
            ...
            for (iColumn = 0; iColumn < numberContinuousColumns; iColumn++) {
                double value = solution[iColumn];
                value = ZteMax(value, saveLower[iColumn]);
                value = ZteMin(value, saveUpper[iColumn]);
                if (solver_->isInteger(iColumn)) {
...
/*                if (handler_->logLevel() > 2) {
          ...
          printf("XX largest infeasibility is %g\n", largestInfeasibility);
                ...
                if (largestInfeasibility > 200.0 * primalTolerance) {
                    handler_->message(CBC_NOTFEAS3, messages_) << largestInfeasibility << CoinMessageEol;
                    objectiveValue = 1.0e50;
                }
            }
        } else {
        ...
        bool solutionComesFromNlp =
                solverCharacteristics_->bestObjectiveValue() < cutoff;
        ...
        // non nlp
        if (!solutionComesFromNlp && fixVariables > 0)
            solverCharacteristics_->setMipBound(-ZTE_DBL_MAX);
        return objectiveValue;
    }
}

// Deals with event handler and solution
ZteEventHandler::CbcAction
ZteModel::dealWithEventHandler(ZteEventHandler::CbcEvent event, double objValue,
    ...
    if (eventHandler) {
        ...
        int numberColumns = solver_->getNumCols();
        double *saveSol = ZteCopyOfArray(bestSolution_, numberColumns);
        if (!saveSol)
            bestSolution_ = new double[numberColumns];
        bestObjective_ = objValue;
        memcpy(bestSolution_, solution, numberColumns * sizeof(double));
        ZteEventHandler::CbcAction action = eventHandler->event(event);
        ...
        return ZteEventHandler::noAction;
    }
}

// Save a solution
void ZteModel::saveExtraSolution(const double *solution,
                                 double objectiveValue) {
    double *save = NULL;
...
void ZteModel::saveBestSolution(const double *solution, double objectiveValue) {
    int n = solver_->getNumCols();
    if (bestSolution_)
...
double ZteModel::getCurrentSeconds() const {
    if (!useElapsedTime())
        return ZteCpuTime() - getDblParam(CbcStartSeconds);
    else
        return ZteGetTimeOfDay() - getDblParam(CbcStartSeconds);
}
...
*/
void ZteModel::setBestSolution(CBC_Message how, double &objectiveValue,
                               const double *solutionIn, int fixVariables)

{

    double *solution = ZteCopyOfArray(solutionIn, solver_->getNumCols());
#ifdef JJF_ZERO
    {
    double saveOffset;
    ...
    if (!solverCharacteristics_->solutionAddsCuts()) {
        ...
        double cutoff = getCutoff();
        if (cutoff < 1.0e30)
            cutoff = ZteMin(cutoff, bestObjective_);
...
nPartiallyFixed %d , nPartiallyFixedBut %d , nUntouched %d\n",
...
#endif
                saveObjectiveValue =
                        ZteMax(saveObjectiveValue,
                               bestObjective_ - 0.0000001 * fabs(bestObjective_));
                cutoff = ZteMin(bestObjective_, saveObjectiveValue) - 1.0e-5;
            ...
            // take a snapshot of current state
        {
            ...
            ZteCopyN(solver_->getColSolution(), numberColBefore, saveColSol);
            // save warm start
        ...
        ZteCopyN(solution, numberColBefore, candidate);
        objectiveValue =
                checkSolution(cutoff, candidate, fixVariables, objectiveValue);
        ...
        if (how == CBC_ROUNDING) {
            ...
            if (ZteMax(currentNumberColCuts, currentNumberRowCuts) > 0) {
                int *which =
                        new int[ZteMax(currentNumberColCuts, currentNumberRowCuts)];
...
void ZteModel::setBestSolution(const double *solution, int numberColumns,
    ...
    if (checkSolution) {
        ...
        double *saveLower = ZteCopyOfArray(solver_->getColLower(), numberColumns);
        double *saveUpper = ZteCopyOfArray(solver_->getColUpper(), numberColumns);
        // Fix integers
        int numberAway = 0;
        for (int i = 0; i < numberColumns; i++) {
            if (solver_->isInteger(i)) {
    ...
    if (cutoff > objectiveValue - increment) {
        ...
        if (cutoffRowNumber_ >= 0) {
            if (solver_->getNumRows() > cutoffRowNumber_) {
                ...
                solver_->setRowUpper(cutoffRowNumber_, cutoff + offset);
                if (continuousSolver_ &&
                    solver_->getNumCols() > continuousSolver_->getNumCols()) {
    ...
    int n = ZteMax(numberColumns, solver_->getNumCols());
    delete[] bestSolution_;
    bestSolution_ = new double[n];
...
void ZteModel::deleteObjects(bool getIntegers) {
    if (ownObjects_) {
        int i;
        for (i = 0; i < numberObjects_; i++)
            delete object_[i];
...
*/
void ZteModel::analyzeObjective()
{
    const double *objective = getObjCoefficients();
    const double *lower = getColLower();
    const double *upper = getColUpper();
    ...
    double smallestObj = ZTE_DBL_MAX;
    {
      ...
      info->addPermutation(permutation);
...
#endif
        int iType = 0;
        if (!numberContinuousObj && numberIntegerObj <= 5 &&
            numberIntegerWeight <= 100 && numberIntegerObj * 3 < numberObjects_ &&
            !parentModel_ && solver_->getNumRows() > 100)
            iType = 1 + 4 + (((moreSpecialOptions_ & 536870912) == 0) ? 2 : 0);
                 ...
                 numberIntegerWeight <= 100 && !parentModel_ &&
                 solver_->getNumRows() > 100 && cost != -COIN_DBL_MAX)
            iType = 4 + (((moreSpecialOptions_ & 536870912) == 0) ? 2 : 0);
        else if (!numberContinuousObj && numberIntegerObj <= 100 &&
                 numberIntegerObj * 5 < numberObjects_ && !parentModel_ &&
                 solver_->getNumRows() > 100 && cost != -COIN_DBL_MAX)
            iType = 8;
        int iTest = getMaximumNodes();
        if (iTest >= 987654320 && iTest < 987654330 && numberObjects_ &&
            !parentModel_) {
            ...
            if (value < lowerValue + 1.0e-6 || value > upperValue - 1.0e-6)
              nAtBound++;
               ...
               single integer variable, that sort of thing.
                    */
                ZteDrand48(true, 1234567);
                for (int i = 0; i < numberColumns; i++) {
                    double lowerValue = lower[i];
                    double upperValue = upper[i];
                    double value = (randomCost) ? ceil((ZteDrand48() + 0.5) * 1000)
                ...
                delete[] fakeObj;
                   ...
                   !numberContinuousObj && !numberGeneralIntegerObj &&
                   numberIntegerObj * 2 < ZteMin(numberColumns, 20)) {
        ...
        for (iRow = 0; iRow < numberRows; iRow++) {
            if (rowLower[iRow] > -1.0e20 &&
                fabs(rowLower[iRow] - rhs[iRow] -
                     floor(rowLower[iRow] - rhs[iRow] + 0.5)) > 1.0e-10) {
                continuousMultiplier = 0.0;
                break;
            }
            if (rowUpper[iRow] < 1.0e20 &&
                fabs(rowUpper[iRow] - rhs[iRow] -
                     floor(rowUpper[iRow] - rhs[iRow] + 0.5)) > 1.0e-10) {
                continuousMultiplier = 0.0;
                break;
            }
                ...
                if (upper[iColumn] > lower[iColumn]) {
                    ...
                    if (objValue && !solver_->isInteger(iColumn)) {
                        ...
                        if (objValue > 0.0) {
                            ...
                            if (lower[iColumn] < -1.0e10 ||
                                fabs(lower[iColumn] - floor(lower[iColumn] + 0.5)) >
                                1.0e-10) {
                                allGood = false;
                                break;
                            } else if (upper[iColumn] < 1.0e10 &&
                                       fabs(upper[iColumn] - floor(upper[iColumn] + 0.5)) >
                                       1.0e-10) {
                                allGood = false;
                                break;
                            }
                            ...
                            for (int j = start; j < end; j++) {
                                ...
                                if (fabs(rhsValue) > 1.0e20 ||
                                    fabs(rhsValue - floor(rhsValue + 0.5)) > 1.0e-10 ||
                                    fabs(element[j]) != 1.0) {
                                    // no good
                                    allGood = false;
                                    break;
                                ...
                                if (element[j] > 0.0) {
                                    if (lowerValue > -1.0e20 &&
                                        fabs(lowerValue - floor(lowerValue + 0.5)) > 1.0e-10) {
                                        // no good
                                        allGood = false;
                                        break;
                                    }
                                    ...
                                    if (upperValue < 1.0e20 &&
                                        fabs(upperValue - floor(upperValue + 0.5)) > 1.0e-10) {
                                        // no good
                                        allGood = false;
                                        break;
                                    }
                            ...
                            if (upper[iColumn] > 1.0e10 ||
                                fabs(upper[iColumn] - floor(upper[iColumn] + 0.5)) >
                                1.0e-10) {
                                allGood = false;
                                break;
                            } else if (lower[iColumn] > -1.0e10 &&
                                       fabs(lower[iColumn] - floor(lower[iColumn] + 0.5)) >
                                       1.0e-10) {
                                allGood = false;
                                break;
    ...
    if ((specialOptions_ & 65536) == 0) {
        ...
        for (iColumn = 0; iColumn < numberColumns; iColumn++) {
            if (upper[iColumn] > lower[iColumn] + 1.0e-8) {
                ...
                value = fabs(lower[iColumn]);
                if (floor(value + 0.5) != value) {
                    scaleFactor = ZteMin(scaleFactor, 0.5);
                    if (floor(2.0 * value + 0.5) != 2.0 * value) {
                        scaleFactor = ZteMin(scaleFactor, 0.25);
    ...
    if (possibleMultiple) {
        for (iColumn = 0; iColumn < numberColumns; iColumn++) {
            if (upper[iColumn] > lower[iColumn] + 1.0e-8) {
                maximumCost = ZteMax(maximumCost, fabs(objective[iColumn]));
            }
        }
    }
    setIntParam(ZteModel::CbcFathomDiscipline, possibleMultiple);
    /*
    ...
    if (possibleMultiple && maximumCost) {
        ...
        if (increment) {
            ...
            double cutoff = getDblParam(ZteModel::CbcCutoffIncrement);
            if (bigIntegers) {
                // allow for 1.0
            ...
            if (value * 0.999 > cutoff) {
                if ((moreSpecialOptions2_ & 65536) == 0) {
                    ...
                    setDblParam(ZteModel::CbcCutoffIncrement,
...
void ZteModel::adjustHeuristics() {
    int numberRows = solver_->getNumRows();
    int numberColumns = solver_->getNumCols();
    int nTree = ZteMax(10000, 2 * numberRows + numberColumns);
    int nRoot = ZteMax(40000, 8 * numberRows + 4 * numberColumns);
    for (int i = 0; i < numberHeuristics_; i++) {
        CbcHeuristicDive *heuristic =
                dynamic_cast<CbcHeuristicDive *>(heuristic_[i]);
        if (heuristic && heuristic->maxSimplexIterations() != ZTE_INT_MAX) {
...
bool ZteModel::maximumSecondsReached() const {
    ...
    if (numberSolutions_ && (!hitMaxTime)) {
        hitMaxTime = totalTime - lastTimeImprovingFeasSol_ >=
                     dblParam_[CbcMaxSecondsNotImproving];
    }
    if (parentModel_ && !hitMaxTime) {
        ...
        if (numberSolutions_ && (!hitMaxTime)) {
            double maxSecondsNotImprFS =
                    parentModel_->getDblParam(CbcMaxSecondsNotImproving);
...
void ZteModel::incrementUsed(const double *solution) {
    if (usedInSolution_) {
            ...
            if (solution[iColumn] < lower[iColumn] + integerTolerance &&
                ...
                if (clpSimplex) {
                   ...
                   lower[iColumn], upper[iColumn]);
                        ...
                        assert(clpSimplex->getColumnStatus(iColumn) ==
...
bool ZteModel::isProvenOptimal() const {
...
bool ZteModel::canStopOnGap() const {
    bool returnCode = false;
    if (bestObjective_ < 1.0e50) {
        double testGap =
                ZteMax(dblParam_[CbcAllowableGap],
                       ZteMax(fabs(bestObjective_), fabs(bestPossibleObjective_)) *
                       dblParam_[CbcAllowableFractionGap]);
        returnCode = (bestObjective_ - bestPossibleObjective_ < testGap &&
                      getCutoffIncrement() >= 0.0);
    }
#if 0
    if (returnCode) {
...
void ZteModel::setOptionalInteger(int index) {
#ifdef CBC_HAS_CLP
    OsiClpSolverInterface *clpSolver =
            dynamic_cast<OsiClpSolverInterface *>(solver_);
    if (clpSolver)
...
void ZteModel::AddIntegers() {
    ...
    int *del = new int[ZteMax(numberColumns, numberRows)];
    ...
    }
    {
        ...
        const int *columnStart =
                continuousSolver_->getMatrixByCol()->getVectorStarts();
        const int *columnLength =
                continuousSolver_->getMatrixByCol()->getVectorLengths();
        ...
        for (int i = 0; i < numberRows; i++) {
            if (!rowLength[i]) {
                ...
                if (rowLength[i] == 1) {
                    ...
                    if (possible) {
                        for (int j = rowStart[i]; j < rowStart[i] + rowLength[i];
                             j++) {
                            int iColumn = column[j];
    ...
    if (copy1->getNumRows() && copy1->getNumCols()) {
...
#endif
        {
            ...
            for (int i = 0; i < numberRows; i++) {
                if (rowLower[i] > -1.0e20 &&
                    fabs(rowLower[i] - floor(rowLower[i] + 0.5)) > 1.0e-12) {
                    couldBeNetwork = false;
                    break;
                }
                if (rowUpper[i] < 1.0e20 &&
                    fabs(rowUpper[i] - floor(rowUpper[i] + 0.5)) > 1.0e-12) {
                    couldBeNetwork = false;
                    break;
                }
                if (possibleRow[i] == 0) {
            ...
            if (couldBeNetwork) {
                ...
                const double *element = matrixByCol->getElements();
                // const int * row = matrixByCol->getIndices();
                const int *columnStart = matrixByCol->getVectorStarts();
                const int *columnLength = matrixByCol->getVectorLengths();
                for (int iColumn = 0; iColumn < numberColumns; iColumn++) {
                    int start = columnStart[iColumn];
                    int end = start + columnLength[iColumn];
...
void ZteModel::addUpdateInformation(const CbcObjectUpdateData &data) {
    if (numberUpdateItems_ == maximumNumberUpdateItems_) {
        maximumNumberUpdateItems_ += 10;
        CbcObjectUpdateData *temp =
                new CbcObjectUpdateData[maximumNumberUpdateItems_];
...
*/
bool ZteModel::doCutsNow(int allowForTopOfTree) const {
...
#define TRY_IDEA1 2
    ...
    if (true && (whenCutsUse < 0 ||
                 (size <= 500 - 500 * TRY_IDEA1 && allowForTopOfTree != 3))) {
...
void ZteModel::setApplicationData(void *appData) { appData_ = appData; }

void *ZteModel::getApplicationData() const { return appData_; }
...
int ZteModel::takeOffCuts(OsiCuts &newCuts, bool allowResolve,
  ...
  if (clpSolver)
    ...
    while (needPurge) {
        ...
        for (i = 0; i < nCuts; i++) {
            ...
            if (status == CoinWarmStartBasis::basic &&
                /*whichGenerator_[i]!=-2*/ newCuts.rowCutPtr(i)->effectiveness() <
                                           1.0e20) {
        ...
        for (i = 0; i < numberNewCuts; i++) {
            ...
            if (status != CoinWarmStartBasis::basic ||
                /*whichGenerator_[i+nCuts]==-2*/ addedCuts[i]->effectiveness() >=
                                                 1.0e20) {
...
void ZteModel::lockThread() {}

void ZteModel::unlockThread() {}

/* Add in any object information (objects are cloned - owner can delete
   originals */
void ZteModel::addObjects(int numberObjects, OsiObject **objects)
{
...
/* Add in any object information (objects are cloned - owner can delete
   originals */
void ZteModel::addObjects(int numberObjects, CbcObject **objects) {
    // If integers but not enough objects fudge
...
*/
void ZteModel::saveModel(OsiSolverInterface *saveSolver,
...
/* Pass in target solution and optional priorities.
   ...
   highest priority */
void ZteModel::setHotstartSolution(const double *solution,
    ...
    if (solution == NULL) {
        ...
        int numberColumns = solver_->getNumCols();
        hotstartSolution_ = ZteCopyOfArray(solution, numberColumns);
        hotstartPriorities_ = ZteCopyOfArray(priorities, numberColumns);
        for (int i = 0; i < numberColumns; i++) {
            if (hotstartSolution_[i] == -ZTE_DBL_MAX) {
...
*/
int ZteModel::cleanBounds(OsiSolverInterface *solver, char *cleanIn) {
    int numberBad = 0;
#ifdef CBC_HAS_CLP
...
#define ZERO_ODD_TOLERANCE 1.0e-14
    ...
    double maxBadHigh = 0.0;
        ...
        for (int i = 0; i < numberColumns; i++) {
          ...
          maxBadHigh = ZteMax(maxBadHigh, solution[i] - columnUpper[i]);
#endif
                } else if (solution[i] < columnLower[i] - ZERO_ODD_TOLERANCE) {
                    numberBad++;
#ifdef SHOW_BAD
                    sumBadLow += columnLower[i] - solution[i];
          maxBadLow = ZteMax(maxBadLow, columnLower[i] - solution[i]);
#endif
                }
            }
        }
...
void ZteModel::deleteNode(CbcNode *node) {
    delete node;
    if (node == currentNode_)
        currentNode_ = NULL;
}

bool ZteModel::stoppingCriterionReached() const {
    return (isNodeLimitReached() ||
            numberSolutions_ >= intParam_[CbcMaxNumSol] || stoppedOnGap_ ||
            eventHappened_ || maximumSecondsReached() ||
            (numberSolutions_ &&
             (intParam_[CbcMaxNodesNotImproving] != ZTE_INT_MAX && (numberNodes_ - lastNodeImprovingFeasSol_ >=
...
*/
bool ZteModel::addCuts1(CbcNode *node, CoinWarmStartBasis *&lastws) {
    int nNode = 0;
...
/*
  ...
  descendants.
*/
int ZteModel::addCuts(CbcNode *node, CoinWarmStartBasis *&lastws)
{
    /*
    ...
    lastws->print();
        ...
        if (currentNumberCuts > 0) {
            ...
            assert(currentNumberCuts + numberRowsAtContinuous_ <=
                   lastws->getNumArtificial());
            assert(currentNumberCuts <=
                   maximumWhich_); // we will read from
            ...
            for (i = 0; i < currentNumberCuts; i++) {
                ...
                if (addedCuts_[i] && (status != CoinWarmStartBasis::basic ||
                                      (addedCuts_[i]->effectiveness() > 1.0e10 &&
                                       !addedCuts_[i]->canDropCut(
                                               solver_, i + numberRowsAtContinuous_)))) {
                 ...
                 numberRowsAtContinuous_ + numberToAdd);
                    ...
                    if ((specialOptions_ & 1) != 0) {
                        ...
                        if (debugger)
                            assert(!debugger->invalidCut(*addedCuts_[i]));
                    }
#endif
                } else {
#ifdef CHECK_CUT_COUNTS
            ...
            if ((specialOptions_ & 4096) == 0) {
                ...
                if (CoinAbs(lastNumberCuts2_ - numberToAdd) < 5) {
                    int numberToCheck = ZteMin(lastNumberCuts2_, numberToAdd);
                    int i1 = 0;
                    int i2 = 0;
                    int nDiff = 0;
                    //int nSame = 0;
          ...
          if (debugger) {
            for (int j = 0; j < numberToAdd; j++)
              assert(!debugger->invalidCut(*addCuts[j]));
            // addCuts[j]->print();
          }
        }
#endif
...
*/
void ZteModel::fillPseudoCosts(double *downCosts, double *upCosts,
                               int *priority, int *numberDown, int *numberUp,
                               int *numberDownInfeasible,
                               int *numberUpInfeasible) const {
    ZteFillN(downCosts, numberIntegers_, 1.0);
    ZteFillN(upCosts, numberIntegers_, 1.0);
    if (priority) {
        ZteFillN(priority, numberIntegers_, 1000000);
    }
    if (numberDown) {
        ZteFillN(numberDown, numberIntegers_, 1);
        ZteFillN(numberUp, numberIntegers_, 1);
    }
    if (numberDownInfeasible) {
        ZteZeroN(numberDownInfeasible, numberIntegers_);
        ZteZeroN(numberUpInfeasible, numberIntegers_);
    }
    int numberColumns = getNumCols();
...
*/
void ZteModel::setObjectiveValue(CbcNode *thisNode,
...
void ZteModel::pseudoShadow(int iActive)
{
            ...
            for (int jColumn = 0; jColumn < numberIntegers_; jColumn++) {
                ...
                for (int j = start; j < end; j++) {
                    ...
                    if (value) {
                        value *= element[j];
                        if (value > 0.0)
                            upValue = ZteMax(upValue, value);
                        else
                            downValue = ZteMax(downValue, -value);
                    }
                }
                up[iColumn] = upValue;
                down[iColumn] = downValue;
        ...
        for (int jColumn = 0; jColumn < numberIntegers_; jColumn++) {
            ...
            for (int j = start; j < end; j++) {
                ...
                if (!useMax)
                    ...
                    weight = ZteMax(weight, thisWeight);
                if (value) {
                    value *= element[j];
                    if (value > 0.0)
                        upValue += value;
            ...
            upValue = ZteMax(upValue, djValue);
            downValue = ZteMax(downValue, -djValue);
            up[iColumn] = upValue;
            down[iColumn] = downValue;
            if (solver_->isInteger(iColumn)) {
                if (!numberNodes_ && handler_->logLevel() > 1)
...
#define PSEUDO_FACTOR 5.0e-1
        ...
        for (int i = 0; i < numberObjects_; i++) {
            ...
            if (obj1 && obj1->upShadowPrice() >= 0.0) {
                ...
                upPseudoCost = ZteMax(pseudoFactor * upPseudoCost, smallUp);
                upPseudoCost = ZteMax(upPseudoCost, up[iColumn]);
                upPseudoCost = ZteMax(upPseudoCost, 0.001 * down[iColumn]);
                ...
                downPseudoCost = ZteMax(pseudoFactor * downPseudoCost, smallDown);
                downPseudoCost = ZteMax(downPseudoCost, down[iColumn]);
                downPseudoCost = ZteMax(downPseudoCost, 0.001 * up[iColumn]);
...
double ZteModel::getBestPossibleObjValue() const
{
    return ZteMin(bestPossibleObjective_, bestObjective_) * solver_->getObjSense();
}
...
*/
bool ZteModel::feasibleSolution(int &numberIntegerInfeasibilities,
...
#define SIMPLE_INTEGER
    ...
    if (infeasibility) {
      ...
      // sumUnsatisfied += infeasibility;
        ...
        double value = solution[iColumn];
        value = ZteMax(value, lower[iColumn]);
        value = ZteMin(value, upper[iColumn]);
        double nearest = floor(value + 0.5);
...
// Set log level
void ZteModel::setLogLevel(int value)
{
...
void ZteModel::incrementStrongInfo(int numberTimes, int numberIterations,
...
void ZteModel::setNumberStrong(int number)
{
    if (number < 0)
        numberStrong_ = 0;
    else
...
void ZteModel::setNumberBeforeTrust(int number)
{
    if (number < -3) {
...
/*! \brief Get an empty basis object
  ...
  result.
*/
CoinWarmStartBasis *ZteModel::getEmptyBasis(int ns, int na) const
...
*/
void ZteModel::assignSolver(OsiSolverInterface *&solver, bool deleteSolver)
...
// Add one heuristic
void ZteModel::addHeuristic(CbcHeuristic *generator, const char *name,
                            int before)
{
...
void ZteModel::setStrategy(CbcStrategy &strategy) {
    delete strategy_;
    strategy_ = strategy.clone();
}

// Set original columns as created by preprocessing
void ZteModel::setOriginalColumns(const int *originalColumns, int numberGood) {
    int numberColumns = getNumCols();
    delete[] originalColumns_;
    originalColumns_ = new int[numberColumns];
    int numberCopy = ZteMin(numberColumns, numberGood);
...
void ZteModel::moveInfo(const ZteModel &rhs) {
    bestObjective_ = rhs.bestObjective_;
    bestPossibleObjective_ = rhs.bestPossibleObjective_;
    numberSolutions_ = rhs.numberSolutions_;
    numberHeuristicSolutions_ = rhs.numberHeuristicSolutions_;
...
void ZteModel::passInPriorities(const int *priorities, bool ifObject) {
    findIntegers(false);
    int i;
    if (priorities) {
        int i0 = 0;
...
void ZteModel::passInTreeHandler(CbcTree &tree) {
    delete tree_;
    tree_ = tree.clone();
}
...
/*
   ...
   then use this
*/
void ZteModel::passInSolverCharacteristics(
        OsiBabSolver *solverCharacteristics) {
    solverCharacteristics_ = solverCharacteristics;
}

void ZteModel::setNodeComparison(CbcCompareBase *compare) {
    delete nodeCompare_;
...
void ZteModel::generateCpp(FILE *fp, int /*options*/) {
    // Do cut generators
    int i;
    for (i = 0; i < numberCutGenerators_; i++) {
        ...
        fprintf(fp, "3  zteModel->addCutGenerator(&%s,%d,", name.c_str(), howOften);
        // change name
        ...
        fprintf(fp, "\"%s\",%s,%s,%s,%d,%d,%d);\n", name.c_str(),
                ...
                whatDepthInSub);
        fprintf(fp, "3  zteModel->cutGenerator(%d)->setTiming(%s);\n", i,
                timing ? "true" : "false");
        fprintf(fp, "3  \n");
    }
    for (i = 0; i < numberHeuristics_; i++) {
    ...
    ZteModel defaultModel;
    ZteModel *other = &defaultModel;
    int iValue1, iValue2;
    double dValue1, dValue2;
    iValue1 = this->getMaximumNodes();
    iValue2 = other->getMaximumNodes();
    fprintf(fp, "%d  int save_getMaximumNodes = zteModel->getMaximumNodes();\n",
            iValue1 == iValue2 ? 2 : 1);
    fprintf(fp, "%d  zteModel->setMaximumNodes(%d);\n",
            iValue1 == iValue2 ? 4 : 3, iValue1);
    fprintf(fp, "%d  zteModel->setMaximumNodes(save_getMaximumNodes);\n",
            iValue1 == iValue2 ? 7 : 6);
    iValue1 = this->getMaximumSolutions();
    iValue2 = other->getMaximumSolutions();
    fprintf(
            fp,
            "%d  int save_getMaximumSolutions = zteModel->getMaximumSolutions();\n",
            iValue1 == iValue2 ? 2 : 1);
    fprintf(fp, "%d  zteModel->setMaximumSolutions(%d);\n",
            iValue1 == iValue2 ? 4 : 3, iValue1);
    fprintf(fp, "%d  zteModel->setMaximumSolutions(save_getMaximumSolutions);\n",
            iValue1 == iValue2 ? 7 : 6);
    iValue1 = this->numberStrong();
    iValue2 = other->numberStrong();
    fprintf(fp, "%d  int save_numberStrong = zteModel->numberStrong();\n",
            iValue1 == iValue2 ? 2 : 1);
    fprintf(fp, "%d  zteModel->setNumberStrong(%d);\n",
            iValue1 == iValue2 ? 4 : 3, iValue1);
    fprintf(fp, "%d  zteModel->setNumberStrong(save_numberStrong);\n",
            iValue1 == iValue2 ? 7 : 6);
    iValue1 = this->numberBeforeTrust();
    iValue2 = other->numberBeforeTrust();
    fprintf(fp,
            "%d  int save_numberBeforeTrust = zteModel->numberBeforeTrust();\n",
            iValue1 == iValue2 ? 2 : 1);
    fprintf(fp, "%d  zteModel->setNumberBeforeTrust(%d);\n",
            iValue1 == iValue2 ? 4 : 3, iValue1);
    fprintf(fp, "%d  zteModel->setNumberBeforeTrust(save_numberBeforeTrust);\n",
            iValue1 == iValue2 ? 7 : 6);
    iValue1 = this->numberPenalties();
    iValue2 = other->numberPenalties();
    fprintf(fp, "%d  int save_numberPenalties = zteModel->numberPenalties();\n",
            iValue1 == iValue2 ? 2 : 1);
    fprintf(fp, "%d  zteModel->setNumberPenalties(%d);\n",
            iValue1 == iValue2 ? 4 : 3, iValue1);
    fprintf(fp, "%d  zteModel->setNumberPenalties(save_numberPenalties);\n",
            iValue1 == iValue2 ? 7 : 6);
    iValue1 = this->howOftenGlobalScan();
    iValue2 = other->howOftenGlobalScan();
    fprintf(fp,
            "%d  int save_howOftenGlobalScan = zteModel->howOftenGlobalScan();\n",
            iValue1 == iValue2 ? 2 : 1);
    fprintf(fp, "%d  zteModel->setHowOftenGlobalScan(%d);\n",
            iValue1 == iValue2 ? 4 : 3, iValue1);
    fprintf(fp, "%d  zteModel->setHowOftenGlobalScan(save_howOftenGlobalScan);\n",
            iValue1 == iValue2 ? 7 : 6);
    iValue1 = this->printFrequency();
    iValue2 = other->printFrequency();
    fprintf(fp, "%d  int save_printFrequency = zteModel->printFrequency();\n",
            iValue1 == iValue2 ? 2 : 1);
    fprintf(fp, "%d  zteModel->setPrintFrequency(%d);\n",
            iValue1 == iValue2 ? 4 : 3, iValue1);
    fprintf(fp, "%d  zteModel->setPrintFrequency(save_printFrequency);\n",
            iValue1 == iValue2 ? 7 : 6);
    iValue1 = this->getPrintingMode();
    iValue2 = other->getPrintingMode();
    fprintf(fp, "%d  int save_printingMode = zteModel->getPrintingMode();\n",
            iValue1 == iValue2 ? 2 : 1);
    fprintf(fp, "%d  zteModel->setPrintingMode(%d);\n",
            iValue1 == iValue2 ? 4 : 3, iValue1);
    fprintf(fp, "%d  zteModel->setPrintingMode(save_printingMode);\n",
            iValue1 == iValue2 ? 7 : 6);
    iValue1 = this->searchStrategy();
    iValue2 = other->searchStrategy();
    fprintf(fp, "%d  int save_searchStrategy = zteModel->searchStrategy();\n",
            iValue1 == iValue2 ? 2 : 1);
    fprintf(fp, "%d  zteModel->setSearchStrategy(%d);\n",
            iValue1 == iValue2 ? 4 : 3, iValue1);
    fprintf(fp, "%d  zteModel->setSearchStrategy(save_searchStrategy);\n",
            iValue1 == iValue2 ? 7 : 6);
    iValue1 = this->specialOptions();
    iValue2 = other->specialOptions();
    fprintf(fp, "%d  int save_cbcSpecialOptions = zteModel->specialOptions();\n",
            iValue1 == iValue2 ? 2 : 1);
    fprintf(fp, "%d  zteModel->setSpecialOptions(%d);\n",
            iValue1 == iValue2 ? 4 : 3, iValue1);
    fprintf(fp, "%d  zteModel->setSpecialOptions(save_cbcSpecialOptions);\n",
            iValue1 == iValue2 ? 7 : 6);
    iValue1 = this->messageHandler()->logLevel();
    iValue2 = other->messageHandler()->logLevel();
    fprintf(fp,
            "%d  int save_cbcMessageLevel = "
            "zteModel->messageHandler()->logLevel();\n",
            iValue1 == iValue2 ? 2 : 1);
    fprintf(fp, "%d  zteModel->messageHandler()->setLogLevel(%d);\n",
            iValue1 == iValue2 ? 4 : 3, iValue1);
    fprintf(
            fp,
            "%d  zteModel->messageHandler()->setLogLevel(save_cbcMessageLevel);\n",
            iValue1 == iValue2 ? 7 : 6);
    iValue1 = this->getMaximumCutPassesAtRoot();
    iValue2 = other->getMaximumCutPassesAtRoot();
    fprintf(fp,
            "%d  int save_getMaximumCutPassesAtRoot = "
            "zteModel->getMaximumCutPassesAtRoot();\n",
            iValue1 == iValue2 ? 2 : 1);
    fprintf(fp, "%d  zteModel->setMaximumCutPassesAtRoot(%d);\n",
            iValue1 == iValue2 ? 4 : 3, iValue1);
    fprintf(
            fp,
            "%d  "
            "zteModel->setMaximumCutPassesAtRoot(save_getMaximumCutPassesAtRoot);\n",
            iValue1 == iValue2 ? 7 : 6);
    iValue1 = this->getMaximumCutPasses();
    iValue2 = other->getMaximumCutPasses();
    fprintf(
            fp,
            "%d  int save_getMaximumCutPasses = zteModel->getMaximumCutPasses();\n",
            iValue1 == iValue2 ? 2 : 1);
    fprintf(fp, "%d  zteModel->setMaximumCutPasses(%d);\n",
            iValue1 == iValue2 ? 4 : 3, iValue1);
    fprintf(fp, "%d  zteModel->setMaximumCutPasses(save_getMaximumCutPasses);\n",
            iValue1 == iValue2 ? 7 : 6);
    iValue1 = this->getPreferredWay();
    iValue2 = other->getPreferredWay();
    fprintf(fp, "%d  int save_getPreferredWay = zteModel->getPreferredWay();\n",
            iValue1 == iValue2 ? 2 : 1);
    fprintf(fp, "%d  zteModel->setPreferredWay(%d);\n",
            iValue1 == iValue2 ? 4 : 3, iValue1);
    fprintf(fp, "%d  zteModel->setPreferredWay(save_getPreferredWay);\n",
            iValue1 == iValue2 ? 7 : 6);
    dValue1 = this->getMinimumDrop();
    dValue2 = other->getMinimumDrop();
    fprintf(fp, "%d  double save_getMinimumDrop = zteModel->getMinimumDrop();\n",
            dValue1 == dValue2 ? 2 : 1);
    fprintf(fp, "%d  zteModel->setMinimumDrop(%g);\n", dValue1 == dValue2 ? 4 : 3,
            dValue1);
    fprintf(fp, "%d  zteModel->setMinimumDrop(save_getMinimumDrop);\n",
            dValue1 == dValue2 ? 7 : 6);
    dValue1 = this->getIntegerTolerance();
    dValue2 = other->getIntegerTolerance();
    fprintf(fp,
            "%d  double save_getIntegerTolerance = "
            "zteModel->getIntegerTolerance();\n",
            dValue1 == dValue2 ? 2 : 1);
    fprintf(fp, "%d  zteModel->setIntegerTolerance(%g);\n",
            dValue1 == dValue2 ? 4 : 3, dValue1);
    fprintf(fp, "%d  zteModel->setIntegerTolerance(save_getIntegerTolerance);\n",
            dValue1 == dValue2 ? 7 : 6);
    dValue1 = this->getInfeasibilityWeight();
    dValue2 = other->getInfeasibilityWeight();
    fprintf(fp,
            "%d  double save_getInfeasibilityWeight = "
            "zteModel->getInfeasibilityWeight();\n",
            dValue1 == dValue2 ? 2 : 1);
    fprintf(fp, "%d  zteModel->setInfeasibilityWeight(%g);\n",
            dValue1 == dValue2 ? 4 : 3, dValue1);
    fprintf(
            fp,
            "%d  zteModel->setInfeasibilityWeight(save_getInfeasibilityWeight);\n",
            dValue1 == dValue2 ? 7 : 6);
    dValue1 = this->getCutoffIncrement();
    dValue2 = other->getCutoffIncrement();
    fprintf(
            fp,
            "%d  double save_getCutoffIncrement = zteModel->getCutoffIncrement();\n",
            dValue1 == dValue2 ? 2 : 1);
    fprintf(fp, "%d  zteModel->setCutoffIncrement(%g);\n",
            dValue1 == dValue2 ? 4 : 3, dValue1);
    fprintf(fp, "%d  zteModel->setCutoffIncrement(save_getCutoffIncrement);\n",
            dValue1 == dValue2 ? 7 : 6);
    dValue1 = this->getAllowableGap();
    dValue2 = other->getAllowableGap();
    fprintf(fp,
            "%d  double save_getAllowableGap = zteModel->getAllowableGap();\n",
            dValue1 == dValue2 ? 2 : 1);
    fprintf(fp, "%d  zteModel->setAllowableGap(%g);\n",
            dValue1 == dValue2 ? 4 : 3, dValue1);
    fprintf(fp, "%d  zteModel->setAllowableGap(save_getAllowableGap);\n",
            dValue1 == dValue2 ? 7 : 6);
    dValue1 = this->getAllowableFractionGap();
    dValue2 = other->getAllowableFractionGap();
    fprintf(fp,
            "%d  double save_getAllowableFractionGap = "
            "zteModel->getAllowableFractionGap();\n",
            dValue1 == dValue2 ? 2 : 1);
    fprintf(fp, "%d  zteModel->setAllowableFractionGap(%g);\n",
            dValue1 == dValue2 ? 4 : 3, dValue1);
    fprintf(
            fp,
            "%d  zteModel->setAllowableFractionGap(save_getAllowableFractionGap);\n",
            dValue1 == dValue2 ? 7 : 6);
    dValue1 = this->getMaximumSeconds();
    dValue2 = other->getMaximumSeconds();
    fprintf(
            fp,
            "%d  double save_cbcMaximumSeconds = zteModel->getMaximumSeconds();\n",
            dValue1 == dValue2 ? 2 : 1);
    fprintf(fp, "%d  zteModel->setMaximumSeconds(%g);\n",
            dValue1 == dValue2 ? 4 : 3, dValue1);
    fprintf(fp, "%d  zteModel->setMaximumSeconds(save_cbcMaximumSeconds);\n",
            dValue1 == dValue2 ? 7 : 6);
}

// Set maximum number of extra saved solutions
void ZteModel::setMaximumSavedSolutions(int value) {
    if (value < maximumSavedSolutions_) {
        for (int i = value; i < maximumSavedSolutions_; i++)
            delete[] savedSolutions_[i];
        maximumSavedSolutions_ = value;
        numberSavedSolutions_ =
                CoinMin(numberSavedSolutions_, maximumSavedSolutions_);
        if (!maximumSavedSolutions_)
            delete[] savedSolutions_;
...
bool ZteModel::isProvenInfeasible() const
{
    if (!status_ && (bestObjective_ >= 1.0e30 &&
...
void ZteModel::initialSolve() {
  ...
  if (clpSolver) {
    ...
    if (solver_->isProvenDualInfeasible())
        originalContinuousObjective_ = -ZTE_DBL_MAX;
    delete[] continuousSolution_;
    continuousSolution_ =
            ZteCopyOfArray(solver_->getColSolution(), solver_->getNumCols());
    setPointers(solver_);
...
*/
int ZteModel::resolve(CbcNodeInfo *parent, int whereFrom, double *saveSolution,
            ...
            if (clpSolver && feasible && !numberNodes_ && false) {
                ...
                double *saveRowUpper = CoinCopyOfArray(rowUpper, numberRows);
                {
                    ...
                    for (int iRow = 0; iRow < numberRows; iRow++) {
                        ...
                        double lowerValue = rowLower[iRow];
                        double upperValue = rowUpper[iRow];
                        if (rowActivity[iRow] < lowerValue + primalTolerance &&
                            djValue > tolerance) {
                            rowUpper[iRow] = lowerValue;
                            assert(clpSimplex->getRowStatus(iRow) != ClpSimplex::basic);
                        } else if (rowActivity[iRow] > upperValue - primalTolerance &&
                                   djValue < -tolerance) {
                ...
                for (int iColumn = 0; iColumn < numberColumns; iColumn++) {
                    ...
                    if (solution[iColumn] < lowerValue + primalTolerance &&
                        djValue > tolerance) {
                        objective[iColumn] = 1.0e8 * direction;
                        assert(clpSimplex->getColumnStatus(iColumn) != ClpSimplex::basic);
                    } else if (solution[iColumn] > upperValue - primalTolerance &&
                               djValue < -tolerance) {
                        objective[iColumn] = -1.0e8 * direction;
                        assert(clpSimplex->getColumnStatus(iColumn) != ClpSimplex::basic);
...
int ZteModel::resolve(OsiSolverInterface *solver) {
    numberSolves_++;
#ifdef CBC_HAS_CLP
    OsiClpSolverInterface *clpSolver =
            dynamic_cast<OsiClpSolverInterface *>(solver);
    ...
    if (clpSolver) {
        ...
        if ((moreSpecialOptions_ & 8388608) == 0)
            ...
            clpSimplex->setSpecialOptions(
                    save |
                    0x11200000); // say is Cbc (and in branch and bound - but save ray)
...
*/
void ZteModel::doHeuristicsAtRoot(int deleteHeuristicsAfterwards) {
    ...
    if (deleteHeuristicsAfterwards != 2) {
        ...
        ZteEventHandler *eventHandler = getEventHandler();
        if (eventHandler)
        ...
        if (!exitNow) {
          ...
          for (int iChunk = 0; iChunk < numberHeuristics_; iChunk += chunk) {
            ...
            for (int i = 0; i < chunk; i++) {
              if (parameters[i].model) {
                ...
                if (heuristic_[i + iChunk]->exitNow(bestObjective_) ||
                    (parameters[i].model->heuristic(0)->switches() &
                     (1024 + 2048)) == (1024 + 2048))
                  exitNow = true;
            ...
            if (!fp && ifSol > 0) {
              ...
              assert(numberWritten == 1);
              numberWritten =
                  fwrite(newSolution, sizeof(double), numberColumns, fp);
              ...
              if (numberColumns == numberColumnsX) {
                ...
                assert(numberRead == 1);
                numberRead =
                    fread(newSolution, sizeof(double), numberColumns, fp);
              ...
              fclose(fp);
                    ...
                    if (ifSol > 0) {
                       ...
                       whereFrom);
                            ...
                            if (eventHandler) {
                                if (!eventHandler->event(ZteEventHandler::heuristicSolution)) {
                    ...
                    if (eventHandler) {
                        if (!eventHandler->event(ZteEventHandler::afterHeuristic)) {
                            eventHappened_ = true; // exit
                            thisSolutionCount = -1000000;
                            break;
                        }
        ...
        if (found >= 0) {
            ...
            if (eventHandler) {
                if (!eventHandler->event(ZteEventHandler::solution)) {
                    eventHappened_ = true; // exit
                }
            }
        }
...
*/
bool ZteModel::solveWithCuts(OsiCuts &cuts, int numberTries, CbcNode *node)
/*
  Parameters:
...
#define STOP_CUTS_NOW 6
	...
	numberTries = CoinMin(numberTries,2);
...
#define CUT_HISTORY 7
    ...
    */
  ZteBaseModel *master = NULL;
  if (numberThreads_ && (threadMode_ & 2) != 0 && !numberNodes_) {
    master = new ZteBaseModel(*this, -1);
  }
#endif
    bool feasible = true;
    int violated = 0;
    ...
    int numberColumns = solver_->getNumCols();
    int numberElementsAtStart = solver_->getNumElements();
  ...
  if ((moreSpecialOptions_ & 4194304) != 0 &&
      solver_->isProvenPrimalInfeasible() && clpSolver &&
      clpSolver->lastAlgorithm() == 2 &&
      clpSolver->getModelPtr()->infeasibilityRay() && !parentModel_) {
    printf("ray exists\n");
  }
#endif
#endif
    ...
    if (node && branchingMethod_) {
        ...
        if (cbcobj && cbcobj->object()) {
            ...
            CbcObjectUpdateData update =
                    object->createUpdateInformation(solver_, node, cbcobj);
      ...
      if (feasible && cutoff < 1.0e20) {
        ...
        if (objectiveChange > 1.0e-5) {
          CbcIntegerBranchingObject *branch =
              dynamic_cast<CbcIntegerBranchingObject *>(cbcobj);
          assert(branch);
          if (way < 0) {
            ...
            OsiIntegerBranchingObject *obj =
                    dynamic_cast<OsiIntegerBranchingObject *>(bobj);
            if (obj) {
                ...
                double objectiveValue = solver_->getObjValue() * solver_->getObjSense();
                double changeInObjective = ZteMax(0.0, objectiveValue - originalValue);
    ...
    if (change > 1.0e-10) {
        dblParam_[CbcSmallestChange] =
                ZteMin(dblParam_[CbcSmallestChange], change);
    ...
    do {
        ...
        if ((currentPassNumber_ == 1 || !numberNodes_) && howOftenGlobalScan_ > 0 &&
            (numberNodes_ % howOftenGlobalScan_) == 0 && (doCutsNow(1) || true)) {
            ...
            if (numberCuts) {
                ...
                int numberColumns = solver_->getNumCols();
                int maximumAdd =
                        ZteMax(numberElements / 10,
                               static_cast<int>(2 * numberColumns)) +
                        100;
                ...
                for (int i = 0; i < numberCuts; i++) {
                    ...
                    if (thisCut->effectiveness() == ZTE_DBL_MAX) {
                ...
                ZteSort_2(violations, violations + numberPossible, which);
                for (int i = 0; i < numberPossible; i++) {
                    ...
                    assert(
                            ...
                            thisCut->effectiveness() == ZTE_DBL_MAX);
#define CHECK_DEBUGGER
#ifdef CHECK_DEBUGGER
                    if ((specialOptions_ & 1) != 0 && !parentModel_) {
                        assert(!solver_->getRowCutDebuggerAlways()->invalidCut(*thisCut));
                    }
#endif
#if 0 // ndef NDEBUG
      ...
      if (clpSolver->getModelPtr()->numberIterations())
        ...
        bool strongCuts =
                (!node && cut_obj[CUT_HISTORY - 1] != -ZTE_DBL_MAX &&
                 fabs(cut_obj[CUT_HISTORY - 1] - cut_obj[CUT_HISTORY - 2]) <
...
#ifdef CBC_THREAD
                status =
            parallelCuts(master, theseCuts, node, slackCuts, lastNumberCuts);
#endif
            }
        ...
        if (eventHandler) {
            ...
            eventHandler->event(ZteEventHandler::generatedCuts);
...
/*                if (solver_->setWarmStart(basis) == false) {
      ...
      if (onOptimalPath && !solver_->isDualObjectiveLimitReached())
                ...
                if (minimumDrop > 0.0) {
                    if (increaseDrop) {
                        ...
                        if (currentPassNumber_ == 13)
                            minimumDrop = ZteMax(1.5 * minimumDrop, 1.0e-5 * fabs(thisObj));
                        else if (currentPassNumber_ > 20 && (currentPassNumber_ % 5) == 0)
                            minimumDrop = ZteMax(1.1 * minimumDrop, 1.0e-5 * fabs(thisObj));
                    ...
                    if (!experimentBreak) {
                        ...
                        double goodDrop = ZTE_DBL_MAX;
                        for (int j = CUT_HISTORY - 1; j >= 0; j--) {
                            if (thisObj - cut_obj[j] < test) {
                                nBadPasses++;
                            } else {
                                goodDrop = (thisObj - cut_obj[j]) /
                                           static_cast<double>(nBadPasses + 1);
                                break;
                            }
                        }
                        maximumBadPasses = ZteMax(maximumBadPasses, nBadPasses);
                        ...
                        double goodDrop =
                                (thisObj - cut_obj[0]) / static_cast<double>(CUT_HISTORY);
        ...
        if (numberTries == 0 && feasible && !keepGoing && !parentModel_ &&
            !numberNodes_) {
            for (int i = 0; i < numberCutGenerators_; i++) {
    ...
    if (feasible &&
        solverCharacteristics_->solutionAddsCuts()) { // check integer feasibility
  ...
  string way = (node == NULL)
                   ? ""
                   : (node->branchingObject()->way()) == 1 ? "Down" : "Up";
  std::cout << "Node " << numberNodes_ << ", father " << fatherNum
    ...
    if (fullScan && numberCutGenerators_) {
        ...
        int numberElementsAdded =
                solver_->getNumElements() - numberElementsAtStart;
        double densityOld = static_cast<double>(numberElementsAtStart) /
                            static_cast<double>(numberRowsAtStart);
        double densityNew = numberRowsAdded
                            ? (static_cast<double>(numberElementsAdded)) /
                              static_cast<double>(numberRowsAdded)
                            : 0.0;
        /*
        ...
        if (!numberNodes_) {
                ...
                if ((specialOptions_ & 1) != 0) {
                    ...
                    if (debugger) {
                        for (int i = 0; i < numberCuts; i++) {
                            ...
                            assert(!debugger->invalidCut(*cut));
                        }
                    }
                }
            }
            ...
            if (thisObjective - startObjective < 1.0e-5 &&
                numberElementsAdded > 0.2 * numberElementsAtStart)
                willBeCutsInTree = -1;
            int whenC = whenCuts_;
            if (whenC == 999999 || whenC == 999998) {
                int size =
                        continuousSolver_->getNumRows() + continuousSolver_->getNumCols();
                ...
                if (thisObjective - startObjective < 1.0e-5) {
                    ...
                    if (numberElementsAdded > 0.2 * numberElementsAtStart) {
                        if (whenCuts_ == 999999) {
                            whenCuts_ = 5000010;
                            if (!smallProblem)
                                maximumCutPasses_ = ZteMax(maximumCutPasses_ >> 1, 1);
                     ...
                     CoinMin(CoinAbs(maximumCutPassesAtRoot_), 8)) {
            if (whenCuts_ == 999999) {
              whenCuts_ = 8000008;
              maximumCutPasses_ = 1;
            } else if (whenCuts_ == 999998) {
              ...
              maximumCutPasses_ = 1;
                     ...
                     CoinMin(CoinAbs(maximumCutPassesAtRoot_), 50)) {
            if (whenCuts_ == 999999) {
              whenCuts_ = 8000008;
              maximumCutPasses_ = 1;
            } else if (whenCuts_ == 999998) {
...
#endif
                    if (currentPassNumber_ < ZteAbs(maximumCutPassesAtRoot_)) {
                        if (whenCuts_ == 999999) {
                            whenCuts_ = 8000008;
                            if (!smallProblem)
                                maximumCutPasses_ = ZteMax(maximumCutPasses_ >> 1, 1);
                        ...
                        if (whenCuts_ == 999999) {
                            whenCuts_ = 10000004;
                            maximumCutPasses_ = ZteMax(maximumCutPasses_, 2);
                        } else if (whenCuts_ == 999998) {
                            whenCuts_ = 11000002;
                            maximumCutPasses_ = ZteMax(maximumCutPasses_, 2);
                        }
        ...
        if ((numberRowsAdded > 100 + 0.5 * numberRowsAtStart ||
            ...
            (densityNew > 200.0 && numberRowsAdded > 100 &&
             densityNew > 2.0 * densityOld)) {
            // much bigger
        ...
        /*
              Noop block 071219.
            */
        if (densityNew > 100.0 && numberRowsAdded > 2 &&
            densityNew > 2.0 * densityOld) {
            // if (thisObjective-startObjective<0.1*fabs(startObjective)+1.0e-5)
            // willBeCutsInTree=-2;
	...
	if (simplex->problemName()!="") // skip if in multiple solvers
	  printf("CbC %s after cuts %d rows, %d columns fixed, obj %g\n",
		 ...
		 numberFixed,simplex->objectiveValue());
            ...
            if (eventHandler_)
                eventHandler_->event(ZteEventHandler::afterRootCuts);
        }
        /*
        ...
        if (!numberNodes_) {
            double value =
                    ZteMax(minimumDrop_, 0.005 * (thisObjective - startObjective) /
                                         static_cast<double>(currentPassNumber_));
            if (numberColumns < 200)
                value = ZteMax(minimumDrop_, 0.1 * value);
#if CBC_USEFUL_PRINTING > 1
...
#ifdef CHECK_KNOWN_SOLUTION
        if (onOptimalPath &&
        (solver_->isDualObjectiveLimitReached() || !feasible)) {
      printf("help 2\n");
    }
#endif
        if (!numberNodes_) {
        ...
        for (i = 0; i < numberCutGenerators_; i++) {
            ...
            /*
               ...
               rows and columns in the second test.
                  */
            if (!probing && howOften == -98 &&
                !generator_[i]->numberShortCutsAtRoot() &&
            ...
            if (howOften == -98 && generator_[i]->switchOffIfLessThan() > 0) {
                if (thisObjective - startObjective <
                    0.005 * fabs(startObjective) + 1.0e-5)
                    howOften = -99; // switch off
                if (thisObjective - startObjective <
                    0.1 * fabs(startObjective) + 1.0e-5 &&
                    5 * solver_->getNumRows() < solver_->getNumCols())
                    howOften = -99; // switch off
            }
            if (generator_[i]->maximumTries() != -1)
                howOften = ZteMin(howOften, -99); // switch off
            /*
            ...
            if (howOften < 0 || howOften >= 1000000) {
                ...
                if (numberNodes_ >= 100000 &&
                    sumChangeObjective1_ > 2.0e2 * (sumChangeObjective2_ + 1.0e-12)) {
                    howOften = 1000000 + SCANCUTS; // wait until next time
            ...
            if (!numberNodes_) {
                ...
                if (howOften >= 0 &&
                    generator_[i]->generator()->mayGenerateRowCutsInTree())
                    willBeCutsInTree = 1;
            }
            /*
        ...
        if (!numberNodes_) {
            ...
            howOften = 0;
            if (howOften) {
                ZTE_DETAIL_PRINT(printf("** method 1\n"));
                // CglProbing * probing =
                // dynamic_cast<CglProbing*>(generator_[iProbing]->generator());
...
*/
int ZteModel::chooseBranch(CbcNode *&newNode, int numberPassesLeft,
    ...
    if (nBranches) {
        ...
        dblParam_[CbcSmallChange] = ZteMax(dblParam_[CbcSmallChange], 1.0e-8);
    } else {
  ...
  if ((specialOptions_ & 1) != 0) {
	  ...
	  testDepth *=2;
        ...
        if (n) {
          ...
          if (!isProvenOptimal()) {
            ...
            if (numberBeforeTrust_ == 0) {
                ...
                anyAction = newNode->chooseDynamicBranch(this, oldNode, reinterpret_cast< OsiSolverBranch *& >(branches),
                                                         numberPassesLeft);
                if (anyAction == -3)
            ...
            OsiBranchingInformation usefulInfo = usefulInformation();
            anyAction =
                    newNode->chooseOsiBranch(this, oldNode, &usefulInfo, branchingState);
        ...
        if (solverCharacteristics_ &&
            ...
            for (int i = 0; i < numberCutGenerators_ && (feasCuts.sizeRowCuts() == 0);
                 i++) {
                if (generator_[i]->normal() && (!generator_[i]->needsOptimalBasis() ||
                                                solver_->basisIsAvailable()))
    ...
    if (anyAction >= 0) {
        ...
        }
        {
            ...
            if (generalBranch) {
                ...
                for (int i = 0; i < numberProblems; i++) {
                    ...
                    newNode2->setDepth(generalBranch->subProblem(i)->depth_ +
                                       currentDepth_);
        ...
        if (newNode->numberUnsatisfied()) {
            maximumDepthActual_ = ZteMax(maximumDepthActual_, newNode->depth());
...
void ZteModel::goToDantzig(int numberNodes, ClpDualRowPivot *&savePivotMethod) {
    // Possible change of pivot method
    if (!savePivotMethod && !parentModel_) {
        OsiClpSolverInterface *clpSolver =
                dynamic_cast<OsiClpSolverInterface *>(solver_);
        if (clpSolver && numberNodes_ >= numberNodes &&
            numberNodes_ < 2 * numberNodes && clpSolver->getNumRows() < 10000) {
...
*/
int ZteModel::doOneNode(ZteModel *baseModel, CbcNode *&node,
                        CbcNode *&newNode) {
    int foundSolution = 0;
  ...
  // node->columnNumber()) ;
    ...
    if (!retCode) {
        ...
        if (solverCharacteristics_->solutionAddsCuts()) {
            ...
            OsiClpSolverInterface *clpSolver =
                    dynamic_cast<OsiClpSolverInterface *>(solver_);
            if ((clpSolver || (specialOptions_ & 16384) != 0) &&
                fastNodeDepth_ < -1 && (specialOptions_ & 2048) == 0) {
#define FATHOM_BIAS (-2)
          ...
          printf("initial depth %d after %d nodes\n",
                ...
                if (node->depth() >= go_fathom &&
                    ...
                    if ((specialOptions_ & 16384) == 0) {
                        ...
                        if (nBranches) {
                            double average =
                                    dblParam_[CbcSumChange] / static_cast<double>(nBranches);
                            info->smallChange_ =
                                    ZteMax(average * 1.0e-5, dblParam_[CbcSmallestChange]);
                            info->smallChange_ = ZteMax(info->smallChange_, 1.0e-8);
                        } else {
                        ...
                        ClpSimplex *simplex = clpSolver->getModelPtr();
                        double *saveLower =
                                ZteCopyOfArray(solver_->getColLower(), numberColumns);
                        double *saveUpper =
                                ZteCopyOfArray(solver_->getColUpper(), numberColumns);
                        if (allSame && false) {
                     ...
                     fastNodeDepth_, FATHOM_BIAS - fastNodeDepth_);
                            ...
                            if (feasible) {
                                ...
                                double *newSolution = ZteCopyOfArray(
                            ...
                            if (feasible) {
                                ...
                                simplex->setObjectiveValue(
                                        CoinMax(bestObjective_, simplex->objectiveValue()));
                                delete[] newSolution;
                                if (bestObjective_ == saveBest) {
                                    if (value > saveBest) {
                                        if ((specialOptions_ & 1) != 0)
            ...
            CPXLPptr lpPtr =
                cpxSolver.getLpPtr(OsiCpxSolverInterface::KEEPCACHED_ALL);
            ...
            if (cpxSolver.isProvenOptimal() && value <= getCutoff()) {
              ...
              // try and do solution
              double *newSolution =
                  CoinCopyOfArray(cpxSolver.getColSolution(), getNumCols());
              setBestSolution(CBC_STRONGSOL, value, newSolution);
              delete[] newSolution;
            }
#endif
      ...
      if ((numberThreads_ == -10 || true) && node->numberBranches() == 2) {
        ...
        if (nodeInfo) {
          ...
          const CbcIntegerBranchingObject *objectI =
              dynamic_cast<const CbcIntegerBranchingObject *>(obj);
          if (objectI) {
            const CbcSimpleInteger *object1 =
                dynamic_cast<const CbcSimpleInteger *>(objectI->object());
            int iColumn1 = -1;
            int way1 = 0;
            ...
            if (object1) {
              ...
              // double check bounds
              assert(bounds1[0] <= lower[iColumn1] &&
                     bounds1[1] >= upper[iColumn1]);
            }
            ...
            while (nodeInfo->parent()) {
              ...
              CbcIntegerBranchingObject *objectI =
                  dynamic_cast<CbcIntegerBranchingObject *>(obj);
              // const OsiObject * object2a = obj->originalObject();
              // assert (object2a);
              const CbcSimpleInteger *object2 =
                  dynamic_cast<const CbcSimpleInteger *>(objectI->object());
              if (nodeInfo->numberBranchesLeft() && object2) {
                ...
                if (way < 0) {
                  ...
                  if (djValue > 1.0e-3 ||
                      solution[iColumn2] < upper[iColumn2] - 1.0e-5) {
                    if (canDelete) {
                      // nRedundantDown++;
#ifndef JJF_ONE
                      COIN_DETAIL_PRINT(
...
#endif
                      if (bounds[0] == bounds[1] || zeroOne2 ||
                          (bounds[0] == lower[iColumn2] && false)) {
                        {
                    ...
                    } else if (iColumn1 >= 0 && iColumn1 != iColumn2 &&
                               (!inBetween || true) && zeroOne1 && zeroOne2 &&
                               false) {
#ifndef JJF_ONE
                      if (true) {
                        // add in bounds
                  ...
                  if (djValue < -1.0e-3 ||
                      solution[iColumn2] > lower[iColumn2] + 1.0e-5) {
                    if (canDelete) {
                      // nRedundantUp++;
#ifndef JJF_ONE
                      COIN_DETAIL_PRINT(
	    ...
	    // end try diving
            ...
            if (newNode->objectiveValue() >= getCutoff()) {
                ...
                // point to useful information
                anyAction =
                        chooseBranch(newNode, numberPassesLeft, node, cuts, resolved,
        ...
        */
        ...
        if (eventHandler_ && !eventHandler_->event(ZteEventHandler::node)) {
            eventHappened_ = true; // exit
        }
        if (parallelMode() >= 0)
        ...
        if (newNode)
            if (newNode && newNode->active()) {
                if (newNode->branchingObject() == NULL) {
                    const double *solution = solver_->getColSolution();
                    ZteEventHandler::CbcAction action = dealWithEventHandler(
                            ZteEventHandler::beforeSolution1, getSolverObjValue(), solution);
                    if (action == ZteEventHandler::addCuts ||
                        ...
                        for (int i = 0; i < numberCutGenerators_; i++) {
                            ...
                            // fixed variables)
                            if (generator_[i]->needsOptimalBasis() &&
                                !solver_->basisIsAvailable())
                                generate = false;
                            if (!generator_[i]->mustCallAgain())
                        ...
                        int numberRowCutsAfter = theseCuts.sizeRowCuts();
                        if (numberRowCutsAfter || action == ZteEventHandler::addCuts) {
                            // need dummy branch
                            ...
                            for (int i=0;i<numberRowCutsAfter;i++) {
                                theseCuts.rowCut(i).setEffectiveness(ZTE_DBL_MAX);
    ...
    if (parallelMode() > 0 && foundSolution) {
        ...
        if (bestObjective_ < baseModel->bestObjective_ &&
            bestObjective_ < baseModel->getCutoff()) {
            baseModel->bestObjective_ = bestObjective_;
            int numberColumns = solver_->getNumCols();
            if (!baseModel->bestSolution_)
                baseModel->bestSolution_ = new double[numberColumns];
            ZteCopyN(bestSolution_, numberColumns, baseModel->bestSolution_);
...
void ZteModel::branchAndBound(int doStatistics) {
#ifdef GET_ALL_SOLUTIONS
    if (dblParam_[CbcCutoffIncrement]==-3333.0) {
    numberHeuristics_=0;
    numberCutGenerators_=0;
    ...
    if (!parentModel_) {
        ...
        */
        if (!dblParam_[CbcStartSeconds]) {
            if (!useElapsedTime())
                dblParam_[CbcStartSeconds] = ZteCpuTime();
            else
                dblParam_[CbcStartSeconds] = ZteGetTimeOfDay();
        }
    }
    dblParam_[CbcSmallestChange] = ZTE_DBL_MAX;
    dblParam_[CbcSumChange] = 0.0;
    dblParam_[CbcLargestChange] = 0.0;
    intParam_[CbcNumberBranches] = 0;
    double lastBestPossibleObjective = -ZTE_DBL_MAX;
    ...
    /*
       ...
       numberBeforeTrust_, and perform integer preprocessing, if requested.
        ...
        We need to hang on to a pointer t
        o solver_. setupOther will assign a
    ...
    if (strategy_) {
        ...
        if (strategy_->preProcessState()) {
            ...
            if (strategy_->preProcessState() < 0) {
                ...
                assert(process);
                process->numberSolvers();
                const int *originalColumns = process->originalColumns();
                // allow for cliques etc
                nOrig = ZteMax(nOrig, originalColumns[numberColumns - 1] + 1);
                ...
                /*
                   ...
                   variables. Make a large enough array for all objects
                        */
                numberObjects_ =
                        numberNewIntegers + numberOldIntegers + numberOldOther + nNonInt;
                object_ = new OsiObject *[numberObjects_];
                delete[] integerVariable_;
    ...
    if (eventHandler) {
        ...
        eventHandler->event(ZteEventHandler::startUp);
    }
#define CLIQUE_ANALYSIS
    ...
    if ((specialOptions_ & 2048) == 0)
        probingInfo_ = new ZteCglTreeProbingInfo(solver_);
    else
        probingInfo_ = NULL;
#else
    probingInfo_ = NULL;
    ...
    if ((specialOptions_ & 64) != 0) {
        ZteCglDuplicateRow dupcuts(solver_);
        dupcuts.setMode(2);
        CglStored *storedCuts = dupcuts.outDuplicates(solver_);
        if (storedCuts) {
            ZTE_DETAIL_PRINT(printf("adding dup cuts\n"));
    ...
    double hotstartObjectiveValue = ZTE_DBL_MAX;
  ...
  solver_->setHintParam(OsiDoReducePrint, false, OsiHintDo, 0);
    ...
    continuousObjective_ = -ZTE_DBL_MAX;
    /*
    ...
    if (!solverCharacteristics_->solverType() ||
        ...
        feasible =
                (solver_->isProvenOptimal() && !solver_->isDualObjectiveLimitReached());
    }
    if (problemFeasibility_->feasible(this, 0) < 0) {
        feasible = false; // pretend infeasible
    }
    ...
    if (!feasible) {
        ...
        if (bestSolution_ &&
            ...
            sprintf(general, "Solution of %g already found by heuristic",
                    trueBestObjValue());
            messageHandler()->message(CBC_GENERAL, messages()) << general << CoinMessageEol;
            ...
            phase_ = 5;
            double increment = getDblParam(ZteModel::CbcCutoffIncrement);
            if ((specialOptions_ & 4) == 0)
                bestObjective_ +=
                        100.0 * increment + 1.0e-3; // only set if we are going to solve
        ...
        delete[] bestSolution_;
        bestSolution_ = new double[numberColumns];
        ZteCopyN(solver_->getColSolution(), numberColumns, bestSolution_);
        return;
    }
    /*
  ...
  if (solver_->getRowCutDebuggerAlways())
    ...
    /*
      ...
      */
    bestObjective_ = ZteMin(bestObjective_, 1.0e50);
    if (!bestSolution_) {
        numberSolutions_ = 0;
        numberHeuristicSolutions_ = 0;
    }
    ...
    if (cutoff < 1.0e20 && direction < 0.0)
        messageHandler()->message(CBC_CUTOFF_WARNING1, messages()) << cutoff << -cutoff << CoinMessageEol;
    ...
    if (clpSolver) {
      ...
      if (!parentModel_)
        clpSolver->getModelPtr()->setSpecialOptions(specialOptions | 32 |
                                                    2097152);
      else
        clpSolver->getModelPtr()->setSpecialOptions(specialOptions &
                                                    ~(32 | 2097152));
    }
  }
#endif
  ...
  if ((moreSpecialOptions2_ & (128 | 256)) != 0) {
    if ((specialOptions_ & 2048) == 0 && numberIntegers_ == numberObjects_) {
      ...
      symmetryInfo_->setupSymmetry(&preModel);
    ...
    if (cutoffRowNumber_ == -2) {
        if (!parentModel_) {
            int numberColumns = solver_->getNumCols();
            double *obj =
                    ZteCopyOfArray(solver_->getObjCoefficients(), numberColumns);
...
/*            if (nTightened)
  ...
  if (numberIntegers_ < solver_->getNumCols())
    ...
    ZteModel **rootModels = NULL;
    rootBundle *bundle = NULL;
    if (!parentModel_ && multipleRootTries_ % 100) {
        double rootTimeCpu = ZteCpuTime();
        double startTimeRoot = ZteGetTimeOfDay();
        ...
        rootModels = new ZteModel *[numberModels];
        bundle = new rootBundle[numberModels];
        int newSeed = randomSeed_;
        if (newSeed == 0) {
            double time = fabs(ZteGetTimeOfDay());
            while (time >= ZTE_INT_MAX)
                time *= 0.5;
        ...
        for (int i = 0; i < numberModels; i++) {
            rootModels[i] = new ZteModel(*this);
...
#define NEW_RANDOM_BASIS
        ...
        if (numberModels > 1 && numberHeuristics_) {
              rootModels[0]->setMoreSpecialOptions(rootModels[0]->moreSpecialOptions() |
                                                   512);
            }
#endif
        delete basis;
        double bestSoFar = ZTE_DBL_MAX;
#ifdef CBC_THREAD
        if (numberRootThreads == 1) {
#endif
        for (int iModel = 0; iModel < numberModels; iModel++) {
            bundle[iModel].
                    model = rootModels[iModel];
            bundle[iModel].
                    iModel = iModel;
            ...
            if (rootModels[iModel]-> getMaximumNodes()) {
              ...
              bundle[iModel].iModel = iModel;
                ...
                for (int iModel = kModel;
                     iModel < CoinMin(numberModels, kModel + numberRootThreads);
                     iModel++) {
...
// to get name
        ...
        for (int i = 0; i < numberSolutions; i++) {
            ...
            setCutoff(1.0e50);
            int iModel = which[i];
            setBestSolution(CBC_ROUNDING, objValue, rootModels[iModel]->bestSolution());
        }
        lastHeuristic_ = NULL;
        delete[] which;
        delete[] value;
...
// Do on switch
    if (doStatistics > 0 && doStatistics <= 100) {
        ...
        memset(statistics_,
               0, maximumStatistics_ * sizeof(CbcStatistics * ));
    }
...
// make sure in sync
    ...
    if (!parentModel_ && testOptions) {
      ...
      while (changed) {
        ...
        symmetryInfo.setupSymmetry(&preModel);
...
/*
  ...
  */
    ...
    currentSolution_ = new double[numberColumns];
    testSolution_ = currentSolution_;
    memcpy(currentSolution_, solver_->getColSolution(), numberColumns * sizeof(double));
// point to useful information
    OsiBranchingInformation usefulInfo = usefulInformation();
...
// add in global cuts from root model
                    ...
                    assert(feasible);
                    {
                        ...
                        for (int i = 0; i < numberColumns; i++) {
                            ...
                            if (tightBounds[2 * i + 0] > oldLower + tolerance) {
                                nTightened++;
                                solver_->
                                        setColLower(i, tightBounds[2 * i + 0]
                                );
                            }
                            if (tightBounds[2 * i + 1] < oldUpper - tolerance) {
                                nTightened++;
                                solver_->
                                        setColUpper(i, tightBounds[2 * i + 1]
                                );
                            }
                        }
                    }
                    ...
                    messageHandler()->message(CBC_GENERAL, messages()) << printBuffer << CoinMessageEol;
                    if (nTightened) {
                        sprintf(printBuffer,
                                "%d bounds were tightened", nTightened);
                        messageHandler()->message(CBC_GENERAL, messages()) << printBuffer << CoinMessageEol;
                    }
                    delete[] solvers;
                }
...
// fixes
                            ...
                            sprintf(printBuffer,
                                    ...
                                    "tightened",
                                    initialCuts, globalCuts_.sizeRowCuts() - initialGlobal, nTightened);
                            messageHandler()-> message(CBC_GENERAL, messages()) << printBuffer << CoinMessageEol;
                            delete[] elements;
                            delete[] indices;
                        }
...
// A) tight bounds on integer variables
                        ...
                        // 输出
                        if (nTightened)
                        ZTE_DETAIL_PRINT(printf("%d tightened by alternate cuts\n", nTightened));
                        if (storedRowCuts_->bestObjective() < bestObjective_) {
// B) best solution
                            double objValue = storedRowCuts_->bestObjective();
                            setBestSolution(CBC_SOLUTION, objValue, storedRowCuts_->bestSolution());
...
#else
                        addCutGenerator(storedRowCuts_,
                                        -99, "Stored from previous run", true,
                                        false, false, -200);
#endif
// Set cuts as active
                        delete[] addedCuts_;
                        ...
                        // 输出
                        ZTE_DETAIL_PRINT(printf("size %d\n", cuts.sizeRowCuts()));
                        cuts = OsiCuts();
...
// User event
    ...
    /*
          ...
          finish off.
        */
      if (feasible && (specialOptions_ & 16384) != 0 && fastNodeDepth_ == -2 &&
          !parentModel_) {
...
/*
  ...
  */
    ...
    if ((fastNodeDepth_ >= 0 || (moreSpecialOptions_ & 33554432) != 0) && /*!parentModel_*/ (specialOptions_ & 2048) == 0) {
// add in a general depth object doClp
        ...
        CbcObject *obj = new ZteGeneralDepth(this, type);
        addObjects(1, &obj);
        delete obj;
...
// Save stuff and return!
        storedRowCuts_-> saveStuff(bestObjective_, bestSolution_, solver_ -> getColLower(), solver_ -> getColUpper());
        delete[] lowerBefore;
        delete[] upperBefore;
        delete saveSolver;
        if (flipObjective)
...
// depth first
        ...
        if (hotstartSolution_ && !hotstartPriorities_) {
              ...
              double direction = solver_->getObjSense();
              int numberColumns = solver->getNumCols();
              double *saveLower = ZteCopyOfArray(solver->getColLower(), numberColumns);
              double *saveUpper = ZteCopyOfArray(solver->getColUpper(), numberColumns);
              // move solution
              ...
              int nFix = 0;
              double bestRatio = ZTE_DBL_MAX;
              for (int iColumn = 0; iColumn < numberColumns; iColumn++) {
                double value = solution[iColumn];
                value = ZteMax(value, saveLower[iColumn]);
                value = ZtenMin(value, saveUpper[iColumn]);
                double sortValue = ZTE_DBL_MAX;
                if (solver->isInteger(iColumn)) {
              ...
              ZTE_DETAIL_PRINT(printf(
                  "** can fix %d columns - best ratio for others is %g on gap of %g\n",
                  nFix, bestRatio, gap));
              int nNeg = 0;
              ZteSort_2(saveLower, saveLower + numberColumns, order);
              for (int i = 0; i < numberColumns; i++) {
                if (saveLower[i] < 0.0) {
                  nNeg++;
              ...
              ZteSort_2(saveLower, saveLower + nNeg, order);
              for (int i = 0; i < numberColumns; i++) {
#if HOTSTART == 1
...
// int numberColumns = solver_->getNumCols();
            ...
            if (numberFixedAtRoot_ > 0) {
                // 输出
                ZTE_DETAIL_PRINT(printf("%d fixed by analysis\n", numberFixedAtRoot_));
                setPointers(solver_);
                numberFixedNow_ = numberFixedAtRoot_;
            } else if (numberFixedAtRoot_ < 0) {
                // 输出
                ZTE_DETAIL_PRINT(printf("analysis found to be infeasible\n"));
                anyAction = -2;
                delete newNode;
                newNode = NULL;
                feasible = false;
        ...
        OsiSolverBranch *branches = NULL;
        if (feasible)
            anyAction = chooseBranch(newNode, numberPassesLeft, NULL, cuts, resolved, NULL, NULL, NULL, branches);
        if (anyAction == -2 || newNode->objectiveValue() >= cutoff) {
            if (anyAction != -2) {
// zap parent nodeInfo
#ifdef COIN_DEVELOP
                printf("zapping ZteNodeInfo %x\n", newNode->nodeInfo()->parent());
#endif
                if (newNode->nodeInfo())
                    newNode->nodeInfo()->nullParent();
            }
...
/*
   ...
   extra cliques from probing info. Not quite worth the effort?
      */
                ...
                ZteThreadRandom randomGenerator;
...
// indicate if doing full search
                ...
                info.randomNumberGenerator = reinterpret_cast<CoinThreadRandom *>(&randomGenerator);
                info.pass = 4;
                generator1.setMode(8);
                OsiCuts cs;
                generator1.generateCutsAndModify(*solver_, cs, &info);
// very clunky
                OsiSolverInterface *temp = dynamic_cast<OsiSolverInterface *>(generator1.cliqueModel(solver_, 2));
                CglPreProcess dummy;
                OsiSolverInterface *newSolver = dynamic_cast<OsiSolverInterface *>(dummy.cliqueIt(*temp, 0.0001));
                delete temp;
                ...
                if (newSolver) {
                    ...
                    int numberCliques = generator1.numberCliques();
                            ...
                            delete [] entry;
#else
                    fake = dynamic_cast<OsiSolverInterface *>(info.analyze(*newSolver, 1, -1));
#endif
                    delete newSolver;
                } else {
                    fake = dynamic_cast<OsiSolverInterface *>(info.analyze(*solver_, 1));
                }
                if (fake) {
// fake->writeMps("fake");
                    ...
                    addCutGenerator(&cliqueGen,
                                    1, "Fake cliques", true, false, false,
                                    -200);
...
// root.
                ...
                addCutGenerator(&implication,
                                1, "ImplicationCuts", true, false, false,
                                -200);
...
// save pointer to root node - so can pick up bounds
            ...
            if (eventHandler) {
                ...
                eventHandler->event(ZteEventHandler::solution);
            }
            delete newNode;
            newNode = NULL;
        }
...
/*
  ...
  */
    ...
    if (numberThreads_) {
        ...
        master_ = new ZteBaseModel(*this, (parallelMode() < -1) ? 1 : 0);
...
// ZteOsiSolverInterface *solver2 = saveSolver->clone();
                    OsiSolverInterface *solver2 = continuousSolver_->clone();
                    const double *lower = saveSolver->getColLower();
                    const double *upper = saveSolver->getColUpper();
                    for (int i = 0; i < numberIntegers_; i++) {
                ...
                for (int i = 0; i < numberIntegers_; i++) {
                    if (objLower[i] > newCutoff) {
                        ...
                        if (objUpper[i] > newCutoff) {
                            newCutoff = -ZTE_DBL_MAX;
                            break;
                        }
// add as global cut
                        objLower[i] = -ZTE_DBL_MAX;
                        OsiRowCut rc;
                        rc.setLb(newLower[i]);
                        rc.setUb(ZTE_DBL_MAX);
                        double one = 1.0;
                        ...
                        n++;
// add as global cut
                        objUpper[i] = -ZTE_DBL_MAX;
                        OsiRowCut rc;
                        rc.setLb(-ZTE_DBL_MAX);
                        rc.setUb(newUpper[i]);
                        double one = 1.0;
                        rc.setRow(1, integerVariable_ + i, &one, false);
                        rc.setGloballyValidAsInteger(2);
                ...
                // 输出
                if (newCutoff == -ZTE_DBL_MAX) {
                    ZTE_DETAIL_PRINT(printf("Root analysis says finished\n")
                    );
                } else if (n > numberFixedNow_) {
                    ZTE_DETAIL_PRINT(printf("%d more fixed by analysis - now %d\n", n - numberFixedNow_, n));
                    numberFixedNow_ = n;
                }
                if (newCutoff != -ZTE_DBL_MAX && n > numberFixedNow_)
                    numberFixedNow_ = n;
...
// redo tree
        ...
        if (numberNodes_ >= lastPrintEvery && ZteWallclockTime() - lastSecPrintProgress_ > secsPrintFrequency_) {
...
// MODIF PIERRE
            ...
            if (parallelMode() > 0 && master_) {
                ...
                for (int i = 0; i < numberThreads; i++) {
                  ZteThread *child = master_->child(i);
                  if (child->node()) {
                    // adjust
...
// Parallel may not have any nodes
                ...
                messageHandler()->message(CBC_STATUS, messages())
                        ...
                        << ZteMax(nNodes, 1)
                        << trueBestObjValue()
                        << trueObjValue(bestPossibleObjective_)
                        << getCurrentSeconds()
                        << CoinMessageEol;
            ...
            if (numberConflictCuts > lastNumberConflictCuts) {
                ...
                printf("%d new conflict cuts - total %d - average length %g\n", numberConflictCuts - lastNumberConflictCuts, numberConflictCuts, length);
                lastNumberConflictCuts = numberConflictCuts;
            }
#endif
            if (eventHandler && !eventHandler->event(ZteEventHandler::treeStatus)) {
                eventHappened_ = true; // exit
            }
            lastSecPrintProgress_ = ZteWallclockTime();
        }
// See if can stop on gap
        if (canStopOnGap()) {
            stoppedOnGap_ = true;
...
#else
        node = dynamic_cast<CbcNode *>(tree_->bestNode(cutoff));
#endif
 ...
 */
        if (stoppedOnGap_) {
            ...
            handler_->message(CBC_MAXTIME, messages_)
                    << CoinMessageEol;
            secondaryStatus_ = 4;
            status_ = 1;
        } else if (numberSolutions_ >= intParam_[CbcMaxNumSol]) {
            handler_->message(CBC_MAXSOLS, messages_)
                    << CoinMessageEol;
            secondaryStatus_ = 6;
            status_ = 1;
        } else if (isNodeLimitReached()) {
            handler_->message(CBC_MAXNODES, messages_)
                    << CoinMessageEol;
            secondaryStatus_ = 3;
            status_ = 1;
        } else if (maximumNumberIterations_ >= 0 && numberIterations_ >= maximumNumberIterations_) {
            handler_->message(CBC_MAXITERS, messages_)
                    << CoinMessageEol;
            secondaryStatus_ = 8;
            status_ = 1;
        } else {
            handler_->message(CBC_EVENT, messages_)
                    << CoinMessageEol;
            secondaryStatus_ = 5;
            status_ = 5;
        }
...
//  If we did any sub trees - did we give up on any?
    ...
    if (eventHandler) {
        eventHandler->event(ZteEventHandler::endSearch);
    }
    if (!status_) {
...
// Conflict cuts
        ...
        if (nConflict) {
            ...
            char general[200];
            sprintf(general,
                    "%d conflict cuts generated - average length %g",
...
// now summary
        ...
        for (i = 0; i < numberNodes2_; i++) {
            ...
            maxDepth = ZteMax(depth, maxDepth);
// Only for completed
            averageNumberIterations1 += statistics_[i]->numberIterations();
            averageValue += value;
            if (endingObjective != ZTE_DBL_MAX && !endingInfeasibility) {
                numberSolutions++;
                averageSolutionDepth += depth;
            }
            if (endingObjective == ZTE_DBL_MAX) {
                numberCutoff++;
...
// was not a good idea to set max time on solvers anyway
        ...
        if (clpSolver && getMaximumSeconds()<1.0e10) {
          OsiClpSolverInterface *continuousSolver = dynamic_cast< OsiClpSolverInterface * >(continuousSolver_);
...
void ZteModel::resizeWhichGenerator(int numberNow, int numberAfter) {
...
#define MAXIMUM_WHICH_MULTIPLIER 2
        ...
        //   CoinMax(maximumWhich_ * MAXIMUM_WHICH_MULTIPLIER +
        //   MAXIMUM_WHICH_INCREMENT, numberAfter), numberAfter);
        maximumWhich_ = ZteMax(maximumWhich_ * MAXIMUM_WHICH_MULTIPLIER +
                               MAXIMUM_WHICH_INCREMENT,
                               numberAfter);
...
int ZteModel::serialCuts(OsiCuts &theseCuts, CbcNode *node, OsiCuts &slackCuts,
                         int lastNumberCuts) {
    /*
        Is it time to scan the cuts in order to remove redundant cuts? If so, set
        up to do it.
  ...
  OsiSolverInterface *cleanLagrangeanSolver = NULL;
	...
	mustResolve =
	  generator_[i]->generateCuts(theseCuts, fullScan, solver_, node,
        ...
        if (!numberRowCutsAfter && !numberColumnCutsAfter &&
            (moreSpecialOptions2_ & 65536) != 0)
            break;
        int j;
