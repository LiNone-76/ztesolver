...
public:
    ...
    inline CbcSolver(const OsiClpSolverInterface &);

    /// Constructor from model
    inline CbcSolver(const CbcModel &);

    /** Copy constructor .
       */
    inline CbcSolver(const CbcSolver &rhs);

    /// Assignment operator
    inline CbcSolver &operator=(const CbcSolver &rhs);
    ...
    /// Add user function
    inline void addUserFunction(CbcUser *function);
    /// Set user call back
    inline void setUserCallBack(CbcStopNow *function);
    /// Add cut generator
    inline void addCutGenerator(CglCutGenerator *generator);
    //@}
    ...
    //@{
    /// Get int value
    inline int intValue(int code);
    /// Set int value
    inline void setIntValue(int code, int value);
    /// Get double value
    inline double doubleValue(int code);
    /// Set double value
    inline void setDoubleValue(int code, double value);
    /// User function (NULL if no match)
    inline CbcUser *userFunction(const char *name) const;
    ...
    /// Copy of model on initial load (will contain output solutions)
    inline void setOriginalSolver(OsiClpSolverInterface *originalSolver);
    /// Copy of model on initial load
    inline void setOriginalCoinModel(CoinModel *originalCoinModel);
...
public:
    ...
    /// Copy constructor
    inline CbcUser(const CbcUser &rhs);

    /// Assignment operator
    inline CbcUser &operator=(const CbcUser &rhs);

    /// Clone
    virtual CbcUser *clone() const = 0;

    /// Destructor
    inline virtual ~CbcUser();
    //@}
...
public:
    ...
    /** Copy constructor .
       */
    inline CbcStopNow(const CbcStopNow &rhs);

    /// Assignment operator
    inline CbcStopNow &operator=(const CbcStopNow &rhs);

    /// Clone
    inline virtual CbcStopNow *clone() const;

    /// Destructor
    inline virtual ~CbcStopNow();
    //@}
...
CBCLIB_EXPORT
inline void CbcMain0(CbcModel &model, CbcParameters &parameters);
CBCLIB_EXPORT
inline int CbcMain1(std::deque<std::string> inputQueue, CbcModel &model,
             CbcParameters &parameters,
             int callBack(CbcModel *currentSolver, int whereFrom) =
             dummyCallback, ampl_info *info = NULL);

static void printGeneralMessage(CbcModel &model, std::string message, int type = CBC_GENERAL);
static void printGeneralWarning(CbcModel &model, std::string message, int type = CBC_GENERAL_WARNING);
CBCLIB_EXPORT
inline int cbcReadAmpl(ampl_info *info, int argc, char **argv, CbcModel &model);
// for backward compatibility (samples)
#define CbcSolverUsefulData CbcParameters
CBCLIB_EXPORT
inline int CbcMain1(int argc, const char *argv[],
