...
#include "CbcSolverHeuristics.cpp"
...
static void printGeneralMessage(CbcModel &model, std::string message, int type)
{
   if (message.length()) {
...
static void printGeneralWarning(CbcModel &model, std::string message, int type)
{
   if (message.length()) {
      model.messageHandler()->message(type, model.messages())
         << message << CoinMessageEol;
...
static CglPreProcess *cbcPreProcessPointer = NULL;
...
inline int CbcClpUnitTest(const CbcModel &saveModel, const std::string &dirMiplib,
...
#else
static void checkSOS(CbcModel * /*babModel*/, const OsiSolverInterface * /*solver*/)
#endif
{
#ifdef COIN_DEVELOP
