...
static inline bool ZteTestFile(const std::string name)
{
...
bool ZteTestMpsFile(std::string &fname)
{
    if (ZteTestFile(fname)) {
        return true;
    }
    if (ZteTestFile(fname + ".mps")) {
        fname += ".mps";
        return true;
    }
    if (ZteTestFile(fname + ".MPS")) {
    ...
    if (CoinFileInput::haveGzipSupport()) {
        if (ZteTestFile(fname + ".gz")) {
            return true;
        }
        if (ZteTestFile(fname + ".mps.gz")) {
            fname += ".mps";
            return true;
        }
        if (ZteTestFile(fname + ".MPS.gz")) {
    ...
    if (CoinFileInput::haveBzip2Support()) {
        if (ZteTestFile(fname + ".bz2")) {
            return true;
        }
        if (ZteTestFile(fname + ".mps.bz2")) {
            fname += ".mps";
            return true;
        }
        if (ZteTestFile(fname + ".MPS.bz2")) {
...
*/
int ZteClpUnitTest(const ZteModel &saveModel, const std::string &dirMiplibIn,
...
//#define CLP_FACTORIZATION_INSTRUMENT
    ...
    for (unsigned int mw = 0; mw < mpsName.size(); mw++) {
        ...
        */
        ...
        double startTime = ZteCpuTime() + ZteCpuTimeJustChildren();
        if (oldStyle) {
            model = new ZteModel(saveModel);
            ...
            if (maximize) {
                ...
                double *array = ZteCopyOfArray(solver1.getObjCoefficients(), numberColumns);
            ...
            model = new ZteModel(solver1);
            ZteMain0(*model, parameters);
            ZteMain1(newInputQueue, *model, parameters, callBack);
        }
        if ((model->getNumRows() != nRows[m] || model->getNumCols() != nCols[m]) && model->getNumRows())
          ...
          delete[] flag;
...
#else
                    modelC->setDualBound(ZteMax(1.0001e9,
                                                ZteMin(1000.0 * largestScaled, 1.0001e10)));
#endif
                }
            ...
            /*
                ...
                (-100). For larger models, use minimum drop to stop (100, 20).
                  */
            model->setMinimumDrop(ZteMin(5.0e-2,
        ...
        model->setMoreSpecialOptions(moreOptions | 4194304);
            ...
            /*
                Finally, the actual call to solve the MIP with branch-and-cut.
                  */
            model->setDblParam(ZteModel::CbcAllowableGap, 0.0001);
      ...
      std::cout << "Factorization " << facTime << " seconds." << std::endl,
        ...
        /*
            ...
            */
        double timeOfSolution = ZteCpuTime() + ZteCpuTimeJustChildren() - startTime;
        std::cout
      ...
      if (implication)
        ...
        */
        ...
        double tolerance = ZteMin(fabs(objActual), fabs(objExpect));
        tolerance = ZteMax(1.0e-4, 1.0e-5 * tolerance);
        if (!model->status()) {
            ...
            if (fabs(objActual - objExpect) < tolerance) {
                std::cout << " (on the bright side solution is correct) ";
            }
        }
    ...
    */
    int returnCode = 0;
    std::cout << "cbc_clp solved " << numProbSolved << " out of " << numberAttempts;
...
#ifdef CLP_FACTORIZATION_INSTRUMENT
    std::cout << "Total factorization time " << timeTakenFac << "seconds." << std::endl;
#endif
    return (returnCode);
}
